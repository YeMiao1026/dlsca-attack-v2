...
E03/E04 的負面結果經 B.18 鑑別診斷後，確認根因是「desync0 調出來的 one-cycle 超參數不適用於 desync 情境」，不是資料或管線問題，也不是 ID 目標本身難學。要解決大機率需要對 desync50/100 各自重新走一輪跟 B.7-B.15 同等規模的調查，尚未投入。E06（cnn_best，見B.20）、E07（resnet，見B.20）模型都已實作完成、能訓練+評估，但都還沒跑正式全量結果、也還沒為各自架構調過 one-cycle 之類的訓練方法論；E02（噪訊增強）還缺動態增強的訓練迴圈才能跑。
