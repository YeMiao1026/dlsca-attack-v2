...
## desync50+resync 完整調查收尾（task #13）

| 階段 | 最佳設定 | GE@1000 | GE@10000 |
|---|---|---|---|
| 起點（B.31） | 未調過的desync0配方 | 117.05 | 83.46（@9000） |
| Phase1 max_lr | lr=1e-3 | 91.70 | 13.00 |
| Phase2 end_percentage | 0.2（預設） | 91.70 | 13.00 |
| Phase3 scale_percentage | 0.1（預設） | 91.70 | 13.00 |
| Phase4 epochs | **30** | **84.97** | **12.00** |

**直接回答「能否贏過E02」：不能，但差距大幅縮小**。E02（N_TGE=206，完全收斂，PI轉正）vs desync50+resync最佳（GE@10000=12.00，用滿額10000條軌跡仍未跌破1，PI仍負）——兩者不在同一信賴層級。四維度全部確認局部最優，投入完整比照desync100的規模，細節見 CLAUDE.md 附錄 B.51-B.54。
