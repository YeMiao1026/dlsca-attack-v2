...
| `E04_desync100_20260816_1858` | resync（max_shift=100）+ 同B.31配方 | 136.04（仍差於隨機） | — | 0.00 | -0.1246 | **negative，resync演算法本身在desync100上幾乎失敗**（估計偏移量跟真實desync相關係數只有-0.0223，desync50是-0.9995）。抖動範圍加倍讓搜尋空間變兩倍，容易鎖錯位置。加大疊代輪數（2→10）有幫助但收斂在-0.45附近，遠不到desync50的水準。下一步應改良resync演算法本身。見 CLAUDE.md 附錄 B.32 |
