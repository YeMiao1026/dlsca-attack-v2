...
### B.66 多 byte 分析：byte 2 是一個**特例**，它的視窗剛好同時裝下兩個必要的洩漏——其他 byte 沒有這個運氣
...
回到 raw 座標掃描 byte 3 的遮罩欄位（`masks[:,1]`）：**遮罩的洩漏峰值在 raw point 98863**（SNR 17.5595），而遮罩值在 **79702**——**兩者相距 19,161 個點**。**任何 700 點視窗都不可能同時裝下**。

**所以 E10 的失敗不是配方的問題，是我抽取視窗的錯誤**——那個視窗讓攻擊在資訊論上就不可能成立，跟超參數毫無關係。**這個錯誤的結論沒有被寫進任何地方，因為在下筆前先做了這項確認。**

**真正的發現，比原本以為的更有意思**：**byte 2 是一個特例**。ANSSI 選的 `[45400, 46100)` 之所以能用單一 700 點視窗支撐 ID 攻擊，是因為 byte 2 的遮罩（raw 45556）與遮罩值（raw 45917）**剛好只差 361 個點**。byte 3 差了 19,161 點。換句話說——**整個 ASCAD 生態系（以及本專案至今的一切）建立在一個運氣很好的 byte 上**；「換一個 byte 重跑」這件事，並不是換個 `target_byte` 參數那麼簡單，而是需要重新做 POI 工程。
...
2. **byte 3 的優勢是它本身洩漏就強得多**，尤其關鍵的是**遮罩的洩漏強了 13.7 倍**（17.4826 vs 1.2737）。ID 攻擊本質上是二階攻擊，必須同時抓到遮罩值與遮罩；byte 2 的遮罩洩漏（1.2737）相對微弱，正是它難攻的原因。
3. **byte 2 其實是一個「雙重特殊」的目標**：既幸運（兩個 POI 只差 361 點，單一 700 點視窗就裝得下——其他 byte 沒這運氣，byte 3 差了 19,161 點），又**偏難**（洩漏強度在測過的兩個 byte 裡明顯較弱）。**整個 ASCAD 生態系與本專案至今的全部結論，都建立在這一個既方便又偏難的 byte 上。**
