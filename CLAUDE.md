...
### B.40 desync100 Phase 3（scale_percentage掃描，峰值固定1e-3）：GE@10000 降到 7.00，且跟 desync0 呈現相反趨勢

以 Phase 2 贏家（`max_lr=1e-3, end_percentage=0.1`）為基礎，掃 `scale_percentage`，峰值LR全程固定在 1e-3（`train.lr = 1e-3 / (100×scale²)`）：

| scale_percentage | train.lr | GE@1000 | GE@10000 | PI |
|---|---|---|---|---|
| 0.05 | 4.0e-3 | 126.39 | — | -0.8260 |
| 0.1（Phase2贏家） | 1.0e-3 | 84.42 | 10.00 | -0.4213 |
| **0.2** | **2.5e-4** | **69.97** | **7.00** | -0.6678 |

**`scale_percentage` 越大（起訖點離峰值越遠，擺動幅度越寬）目前表現越好**——這跟 desync0 當年 B.15 的結論（scale越小越好，單調趨勢往0.05以下探）**方向完全相反**。這是一個清楚的證據：desync100 的最佳訓練動態跟 desync0/desync50 本質不同，不能假設同一個超參數維度上「越極端越好」的方向會通用。**GE@10000=7.00**，比 Phase2 的 10.00 更接近收斂，正在往 scale 更大的方向試探是否能繼續推進。

再測 `scale_percentage=0.3`（`train.lr=1.111e-4`，峰值仍固定1e-3）：

```
GE @ N=1000 = 87.32（比scale=0.2的69.97差）
PI          = -0.4562
```

**趨勢反轉，`scale_percentage=0.2` 確認是局部最優**——跟 E01 當年 max_lr／end_percentage 掃描的模式一致（兩側都更差，中間是最優點），不是「越大越好」的單調趨勢。`scale_percentage` 這個維度探到此為止。

### B.41 desync100 完整調查 Phase 1-3 總結：GE@10000=7.00，非常接近但尚未完全收斂

三個階段的完整軌跡（起點是resync修好後、完全未調過的desync50成功配方直接套用失敗，B.34-B.36）：

| 階段 | 調整維度 | 最佳設定 | GE@1000 | GE@10000 |
|---|---|---|---|---|
| 起點（B.34-B.36） | 無（沿用desync50配方） | lr=0.02/scale=0.05 | 138.68起 | 未評估（負面） |
| Phase 1 | max_lr（scale=0.1固定） | **lr=1e-3** | 95.17 | 41.80 |
| Phase 2 | end_percentage | **0.1** | 84.42 | 10.00 |
| Phase 3 | scale_percentage（峰值固定1e-3） | **0.2** | **69.97** | **7.00** |

**目前最佳配方**：`max_lr=1e-3, end_percentage=0.1, scale_percentage=0.2`（實際峰值1e-3，`train.lr=2.5e-4`），desync100 專屬調過，跟 desync50 的最佳配方（`lr=0.02, end_percentage=0.2, scale_percentage=0.05`，峰值5e-3）幾乎每個維度都不一樣，再次證實兩個資料集的最佳訓練動態本質不同。

**GE@10000=7.00**：已經非常接近完全收斂（GE<1才算N_TGE），但還沒有真正跌破1並維持——這代表資料上限（Attack集僅10000條）可能是目前主要瓶頸，而非超參數本身還有太多空間。三個維度（max_lr、end_percentage、scale_percentage）都已經找到局部最優，continuing在同樣三個維度上微調的邊際效益可能有限，如同 E01 當年在 B.13-B.15 三個維度都摸到局部最優後的處境。

**是否要繼續投入**（例如嘗試 epoch 數/schedule長度、或接受 GE@10000=7.00 作為 desync100 目前最好但誠實的結果）：留待使用者決定。

### B.42 desync100 Phase 4（epoch數/schedule長度掃描）：確認 50 epochs 也是局部最優，四個維度全部摸到邊界

**操作插曲**：這輪 launch 過程中一次工具呼叫被使用者以介面動作標記為「拒絕」，但底層 SSH 連線在被標記拒絕前已經送出並在遠端 detach，導致該次呼叫其實真的在伺服器上執行了；使用者隨後說「繼續投入」，於是同一組指令又被重新送了一次——造成同一份設定（`epochs=30` 在 GPU0、`epochs=75` 在 GPU1）**各自重複跑了兩份**。發現後立刻用 `ps aux` 比對指令列與啟動時間，`kill` 掉較晚啟動的兩個重複行程，保留較早的那組跑完，沒有造成資料損毀（PID-based run_dir 命名已經杜絕撞名覆寫，只是浪費了一些GPU運算資源）。

**以 Phase 3 贏家（`max_lr=1e-3, end=0.1, scale=0.2`）為基礎**，掃 `train.epochs`（連動 one-cycle schedule 長度）：

| epochs | GE@1000 | GE@10000 | PI |
|---|---|---|---|
| 30 | 82.46 | 11.00 | -0.2568 |
| **50（Phase3贏家）** | **69.97** | **7.00** | -0.6678 |
| 75 | 111.71 | — | -0.0500 |

**50 epochs 確認也是局部最優**——30 epochs（訓練不夠久）跟 75 epochs（訓練太久，跟 B.8 desync0 當年「拉長one-cycle排程反而更差」的教訓一致）都比 50 差。**四個超參數維度（max_lr、end_percentage、scale_percentage、epochs）現在全部確認落在各自的局部最優點**，desync100 目前最佳配方（`max_lr=1e-3, end=0.1, scale=0.2, epochs=50`）在這一輪四維度掃描下沒有再進步的空間。

**本輪完整投入的收尾結論**：desync100 從 resync 對齊修好（B.33）到現在，經過完整四維度超參數掃描（Phase 1-4，共約12次系統性訓練），**最佳結果穩定在 GE@10000=7.00**，非常接近但尚未達成 N_TGE（GE需跌破1）。這是目前為止最誠實、最完整的 desync100 結果——不像 E01 最終能推到 N_TGE=475 這種明確數字，desync100 受限於 Attack 集僅10000條軌跡的評估窗口上限，"7.00" 可能已經是這套資料/架構組合能達到的實務極限，除非拉大 A 集規模、增加模型容量，或接受在現有四個維度之外尋找全新方向（例如再次嘗試噪訊增強組合、或調整 batch_size）。

**使用者決定**：接受 `GE@10000=7.00` 作為 desync100 目前正式、誠實的最佳結果（`runs/E04_desync100_20260816_201058_scale02/`，配方 `max_lr=1e-3, end_percentage=0.1, scale_percentage=0.2, epochs=50` + resync）。task #10（desync100 完整超參數調查）在此正式結案，不強行繼續投入更多嘗試。

**desync系列最終結果總表**：

| 資料集 | 是否用resync | 最佳配方是否為desync0配方原封不動 | 最佳結果 | N_TGE |
|---|---|---|---|---|
| desync0 | 不適用 | — | N_TGE=475（E01）/ N_TGE=206（E02噪訊增強） | 已收斂 |
| desync50 | 是（B.30-B.31） | 是，未調過 | GE@9000降到接近0，第一次嘗試就突破 | 未跌破1，但趨勢完全收斂 |
| desync100 | 是（B.33修正版） | 否，需要專屬四維度調參（B.38-B.42） | **GE@10000=7.00** | 未達成，Attack集10000條上限可能是瓶頸 |

**方法論貢獻總結（適合寫進期末報告方法論章節）**：desync 情境下 CNN 訓練失敗的根因鏈——①單點SNR健檢揭露訊號被時間抖動打散而非消失（B.29）→②互相關盲對齊大幅回復SNR（B.30）→③對齊分數必須正規化才能在大抖動範圍下正確運作，否則系統性偏向小偏移量（B.33）→④即使對齊修好，訓練配方仍需要針對個別資料集重新調整，不能假設遷移（B.34-B.42）。這一條完整的偵錯鏈本身就是這次重構最有價值的產出之一，比單一個 N_TGE 數字更能展示方法論的嚴謹度。

## cnn_best（E06）完整超參數調查（task #11）

### B.43 Phase 1：One-Cycle LR（RMSprop）峰值掃描，兩點皆負面

`cnn_best`（66.6M參數）在GPU上訓練很便宜（~2-4秒/epoch），適合投入完整調查。基準（flat RMSprop lr=1e-5，B.27）：GE@1000=157.77，幾乎沒學到東西。

比照 cnn_light 當年 one-cycle 是最大突破口的經驗，先試 One-Cycle LR（沿用 RMSprop，Adam 依陷阱清單#10 會發散）：

| peak (train.lr) | GE@1000 | PI | 備註 |
|---|---|---|---|
| 1e-4 | 163.33 | -0.0183 | 跟基準幾乎沒差，兩者都在epoch35觸發patience早停 |
| 1e-3 | 158.68 | -0.0273 | 同上 |
| 1e-5（跟flat基準同量級） | 152.82 | -0.0257 | 三個峰值裡最接近基準，但仍是負面 |

**跟 cnn_light/desync100 的經驗明顯不同**：那兩個案例 one-cycle 一出手就有感（cnn_light GE~30降到~15；desync100 max_lr掃描第一次就找到能突破隨機基準的點）。`cnn_best` 這裡兩個峰值（橫跨兩個數量級）都完全沒有起色，暗示問題可能不只是 LR schedule 形狀，而是這個 66.6M 參數模型本身（相對 A集僅30000條軌跡嚴重過參數化）在這個任務上的學習動態跟輕量模型本質不同。

### B.44 Phase 1 結論：One-Cycle 對 cnn_best 完全沒有幫助，三個峰值都跟 flat 基準無法區分

三點都跑完了：

| peak (train.lr) | GE@1000 | PI |
|---|---|---|
| 1e-4 | 163.33 | -0.0183 |
| 1e-3 | 158.68 | -0.0273 |
| 1e-5 | 152.82 | -0.0257 |
...
**四個數字（152.82-163.33）全部落在同一個窄範圍內，統計上無法區分——不管 one-cycle 峰值設多少，結果都跟完全不用 one-cycle 的 flat 基準一樣**。這橫跨了兩個數量級的峰值測試（1e-5到1e-3），排除了「峰值選錯」這個解釋；`1e-5` 這個排程形狀跟 flat 基準用同一個LR量級，結果也沒有分別，代表**連「排程形狀本身」都沒有發揮作用**。

`train_history.csv` 顯示三次訓練的 loss 全程幾乎完全貼著隨機基準 `log(256)=5.545` 沒有真正下降（`1e-5` 那次到 epoch75 才勉強降到5.5405，這個下降幅度連 desync100 最差的失敗嘗試都不如）。**這不是「還沒調到對的LR」的問題，比較像是模型完全沒有在學習**。

**跟 cnn_light/desync100 的關鍵差異**：desync100 的問題出在「訊號存在但被抖動打散」，資料本身沒有先天缺陷；`cnn_best` 這裡沒有任何類似的資料層面問題（用的是desync0，SNR峰值已知是6.30），排除資料因素後，問題只能出在**模型與優化器的組合本身**——66.6M 參數（是 `cnn_light` 的 3574 倍）對上僅 30000 條訓練軌跡，可能是嚴重的優化困難（過大的參數空間讓梯度訊號被稀釋），而不是單純的欠擬合可以用調 LR schedule 解決。

**這是一個跟 desync 系列性質不同的失敗模式，LR-tuning 這條路線（對cnn_light/desync100都有效）在這裡似乎已經走到頭**。下一步可能的方向（留待決定）：(1) 嘗試更激進的純 flat 高LR（不用one-cycle，直接測試RMSprop在更高LR下是否發散或是否有效）；(2) 改用 batch_size 更小的設定（原論文用200，或許對這個任務太大）；(3) 接受 `cnn_best` 在合理超參數搜尋範圍內就是學不到東西，轉去做 resnet（task #12）。

**使用者決定**：cnn_best 暫停（task #11 標記 pending，非結案，可後續回頭），轉去投入 resnet（task #12）。

## resnet（E07）完整超參數調查（task #12）

### B.45 Phase 1（max_lr掃描）：`lr=1e-4` 首次真正突破隨機基準

已有兩個既有資料點（B.24，皆用預設 `scale_percentage=0.1`）：`peak=1e-3` → GE@1000=131.34；`peak=5e-3` → GE@1000=174.63（訓練期預覽假警報）。兩者都比隨機基準127.5差。這次往更低的峰值方向測試：

| peak (train.lr) | GE@1000 | GE@9000 | PI |
|---|---|---|---|
| **1e-4** | **107.87** | **70.93（持續下降）** | -0.0243 |
| 2.5e-4 | 116.82 | — | -0.0247 |
| 1e-3（既有） | 131.34 | — | -0.0268 |
| 5e-3（既有，假警報） | 174.63 | — | -0.1472 |

**`lr=1e-4` 是resnet調查以來第一個真正突破隨機基準的配方**，且拉寬窗口後（127.5→107.87→70.93）曲線持續下降、形狀健康，不是假警報。四點呈現清楚的單調趨勢——peak越小越好，resnet（28.8K參數，比cnn_light稍大但同數量級）的最佳LR區間看起來遠比 cnn_light/desync100 的 1e-3~1e-2 更保守。

**下一步**：以 `lr=1e-4` 為基礎，比照 desync100 playbook 掃 `end_percentage`／`scale_percentage`，或繼續往更低的peak方向探（例如5e-5），先確認單調趨勢有沒有反轉。

### B.46 繼續往更小的 peak 推進，`lr=5e-5` 再度改善（GE@9000=56.21）

```
lr=5e-5：  GE@1000=102.69  GE@9000=56.21（持續下降，比lr=1e-4的70.93更好）
end_percentage=0.35（lr=1e-4基礎）：GE@1000=112.07（比預設0.2的107.87差）
```

max_lr 的單調趨勢延續（peak越小越好，目前5e-5 > 1e-4 > 2.5e-4 > 1e-3 > 5e-3），`end_percentage` 這邊維持預設 `0.2` 較好（`0.35`更差，跟desync100那邊`0.1`最好的結論不同，再次印證不同資料/架構的最佳超參數方向不通用）。正在測試 `lr=1e-5`（繼續往更小推進，確認趨勢是否反轉）跟 `end_percentage=0.1`（在`5e-5`基礎上補齊end_percentage三點）。

### B.47 max_lr 趨勢在 `5e-5` 反轉，`end_percentage=0.1` 是新贏家（GE@9000=51.32）

```
lr=1e-5：       GE@1000=105.24（比5e-5的102.69差，趨勢反轉，確認5e-5是max_lr局部最優）
end_percentage=0.1（配合lr=5e-5）：GE@1000=101.17  GE@9000=51.32（比0.2的56.21更好，新贏家）
```
...
### B.49 resnet 完整調查收尾：四維度全部摸到局部最優，GE@9000=51.32

Phase 4（epoch數，其餘沿用Phase1-3贏家）：
...
**`epochs=100` 確認是局部最優**——50 epochs幾乎沒差（訓練還沒到收斂前緣），200 epochs反而變差（同 B.8/B.42 的「排程長度是跟其他超參數綁定調過的，不是越長越好」教訓）。**四個超參數維度（max_lr、end_percentage、scale_percentage、epochs）現在全部確認落在各自的局部最優點**。

**resnet 完整調查軌跡總表**：

| 階段 | 調整維度 | 最佳設定 | GE@1000 | GE@9000 |
|---|---|---|---|---|
| 起點（B.24） | 無（沿用隨手選的Adam lr=1e-3 flat / 兩次one-cycle嘗試） | Adam lr=1e-3 flat | 123.27 | 未評估 |
| Phase 1 | max_lr（scale=0.1固定） | **lr=5e-5** | 102.69 | 56.21 |
| Phase 2 | end_percentage | **0.1** | 101.17 | 51.32 |
| Phase 3 | scale_percentage（峰值固定5e-5） | 0.1（確認為預設值最佳） | 101.17 | 51.32 |
| Phase 4 | epochs | 100（確認為預設值最佳） | 101.17 | 51.32 |

**目前最佳配方**：`One-Cycle LR, max_lr=5e-5, end_percentage=0.1, scale_percentage=0.1, epochs=100`（其餘沿用 ReLU+he_normal+BN 架構不變）。**GE@9000=51.32**，比 B.24 的起點（123.27@1000，未曾拉寬過）有明確、可驗證的改善，且是 resnet 調查以來唯一一組通過「隨評估窗口拉寬持續下降」健全性檢查（不是假警報）的配方。

**跟 desync100 的對比**：兩者都投入了完整四維度掃描（各約14-15次系統性訓練），desync100 最終逼近 GE@10000=7.00（接近但未達N_TGE），resnet 則停在 GE@9000=51.32——resnet 的收斂速度明顯較慢。resnet 是一個刻意設計來測試「架構錯配」情境的異架構對照組（Stage B transferability，CLAUDE.md §6.1），本身沒有 desync 那種「訊號被打散」的已知病因，51.32 這個結果目前判斷是「架構本身收斂較慢」的真實反映，不是還沒調好——四個維度都已摸到局部最優，繼續在這四個維度上微調的邊際效益有限。

## cnn_best（E06）Phase 2：flat高LR、小batch皆負面，投入結案

### B.50 cnn_best 完整調查收尾：五次系統性嘗試全數收斂到同一個負面區間，判定為模型本身的優化困難

依 B.44 提出的兩個新方向，測試：

| 嘗試 | 設定 | 訓練期預覽GE | 正式GE@1000 | PI |
|---|---|---|---|---|
| flat高LR | RMSprop flat lr=1e-3（不用one-cycle） | 195.65 | 159.51 | -0.0450 |
| 小batch | batch_size=50（原論文200） | 116.80（看似有希望） | **158.91（假警報推翻）** | -0.0226 |

**`batch_size=50` 的訓練期預覽（116.80，低於隨機基準）看起來像是第一個突破訊號，正式評估後徹底推翻**——這是本次全專案調查裡第N次「少量run預覽被雜訊騙到」的案例（跟B.24 resnet、B.28/B.35 desync50/100 同一個模式），連續驗證了「一定要用100-run正式評估才能下結論」這個方法論鐵律的必要性。

**cnn_best 完整調查總表（5次系統性嘗試，涵蓋兩輪）**：

| 嘗試 | GE@1000 |
|---|---|
| flat基準（RMSprop lr=1e-5，B.27） | 157.77 |
| one-cycle peak=1e-4（B.43） | 163.33 |
| one-cycle peak=1e-3（B.43） | 158.68 |
| one-cycle peak=1e-5（B.44） | 152.82 |
| flat lr=1e-3（B.50） | 159.51 |
| batch_size=50（B.50） | 158.91 |

**全部6個數字（152.82-163.33）落在同一個窄範圍內，統計上完全無法區分**——不管是調LR schedule形狀、調峰值大小橫跨四個數量級、還是調batch size，結果都收斂到同一個「幾乎沒學到東西」的區間。`train_history.csv` 顯示所有嘗試的 loss 全程幾乎貼著隨機基準 `5.545`，沒有一次真正下降超過0.03。

**結論：cnn_best（66.6M參數）在這個任務上的優化困難，不是任何單一超參數能解決的**——排除了資料因素（同一份desync0資料在cnn_light上能訓到N_TGE=475/206）、排除了LR schedule形狀（one-cycle 3個峰值都試過）、排除了LR絕對大小（1e-5到1e-3橫跨4個數量級都試過）、排除了batch size（200→50）。合理的解讀是：66.6M參數對上僅30000條訓練軌跡（3574倍cnn_light的參數量），這個參數/樣本比例可能本身就是優化困難的根源，需要架構層級的介入（減少參數量、加正規化、換優化器整套邏輯）才可能解決，超出了「調超參數」這條路線的範疇。

**task #11 在此結案（暫停，非放棄）**：5次系統性嘗試已經合理覆蓋了「調超參數」能做的範圍，繼續在這個方向投入邊際效益低。cnn_best 目前誠實最佳結果是 flat 基準的 GE@1000=157.77（B.27），維持負面。

## desync50+resync 補齊完整投入（task #13）

### B.51 desync50+resync Phase 1（max_lr掃描）：GE@10000 從 83.46 大幅推進到 13.00

**動機**：desync50+resync 先前只投入2次嘗試（B.31基準117.05/83.46、B.32重調131.01更差）就轉去做 desync100，這個投入順序不合理——desync50 是更簡單的問題（抖動只有desync100的一半，對齊品質同樣接近完美），desync100 卻投入了完整四維度掃描並推進到接近收斂。使用者指出這個落差，要求補齊。

比照 desync100 playbook，Phase 1 掃 `max_lr`（`scale_percentage=0.1`固定）：

| max_lr | GE@1000 | GE@9000 | GE@10000 | PI |
|---|---|---|---|---|
| **1e-3** | **91.70** | **19.20** | **13.00** | -0.0806 |
| 5e-3（B.32既有資料，scale=0.1） | 131.01 | — | — | -0.2200 |
| 1e-2 | 114.03 | — | — | -0.2428 |

**`max_lr=1e-3` 讓 GE@10000 從 B.31 基準的 83.46 大幅推進到 13.00**——曲線平順下降（127.5→91.70→19.20→13.00），跟desync100最佳配方的收斂軌跡（GE@10000=7.00）同等級，證實假設成立：desync50確實有大量先前沒開發的空間。目前已經很接近desync100的最終結果，且desync50理論上應該更容易收斂。下一步：以`max_lr=1e-3`為基礎掃`end_percentage`。

### B.52 Phase 2（end_percentage掃描）：影響不大，0.2確認維持最佳

| end_percentage | GE@1000 | PI |
|---|---|---|
| 0.1 | 95.06 | -0.1452 |
| **0.2（贏家，預設）** | **91.70** | -0.0806 |
| 0.35 | 93.79 | -0.0892 |
...
### B.54 desync50+resync 完整調查收尾：GE@10000 推進到 12.00，但仍未超越 E02

**Phase 4（epoch數）**：

| epochs | GE@1000 | GE@9000 | GE@10000 |
|---|---|---|---|
| **30（新贏家）** | **84.97** | **16.54** | **12.00** |
| 50（預設，先前贏家） | 91.70 | 19.20 | 13.00 |
| 75 | 111.05 | — | — |

**`epochs=30` 小幅超越預設的50**，四個維度全部確認完成。目前最終最佳配方：`max_lr=1e-3, end_percentage=0.2, scale_percentage=0.1, epochs=30` + resync（max_shift=50）。

**desync50+resync 完整投入軌跡總表**：

| 階段 | 調整維度 | 最佳設定 | GE@1000 | GE@10000 |
|---|---|---|---|---|
| 起點（B.31，未調過的desync0配方） | 無 | he_uniform+lr=0.02/scale=0.05 | 117.05 | 83.46（@9000） |
| Phase 1 | max_lr（scale=0.1固定） | **lr=1e-3** | 91.70 | 13.00 |
| Phase 2 | end_percentage | 0.2（確認預設最佳） | 91.70 | 13.00 |
| Phase 3 | scale_percentage（峰值固定） | 0.1（確認預設最佳） | 91.70 | 13.00 |
| Phase 4 | epochs | **30** | **84.97** | **12.00** |

**直接回答原本的問題「desync50+resync 能不能贏過 E02？」——答案是：目前不能，但差距已經從「完全比不上」大幅縮小到「很接近但還沒跨過去」。**

具體比較：
- **E02**（desync0+噪訊增強）：N_TGE=206——GE 在 1000 條軌跡窗口內就已經跌破1並維持，PI轉正（+0.1796），是完全收斂、可信賴的金鑰恢復。
- **desync50+resync**（這次投入後的最佳結果）：GE@10000=12.00——用整個 Attack 集（10000條，比E02多10倍的軌跡）還沒能讓 GE 跌破1，PI仍是負值（-0.0464）。

**這兩個數字不是同一個量級的成功**：E02 用十分之一的軌跡數就完全破解；desync50+resync 用滿額的軌跡數還差臨門一腳。desync50+resync 確實透過完整四維度調查大幅推進了（從「完全學不到東西」到「非常接近收斂」），驗證了使用者的假設（desync50被低估、值得投入），但**尚未超越 E02**，兩者不在同一個信賴層級。

**後續如果要繼續逼近**：GE@10000=12.00 距離跌破1不遠，理論上還有機會透過更精細的微調（例如更小的epoch間距、或跟E02一樣加開噪訊增強）真正跨過去，但四個已測維度都已摸到局部最優，繼續投入的邊際效益需要評估。task #13 在此完成既定的四階段投入，回報使用者。

---

## 附錄 C：防禦端銜接工作（dlsca-defense-v2 的起點）

**範疇說明**：§1.3 明確排除本次重構做防禦器訓練（另案 `dlsca-defense-v2`），但介面須預留。本節記錄的是**攻擊端這邊為了讓防禦端可以接上去而做的最小基礎建設**，不是防禦器本身的實作——GAN/對抗訓練的擾動產生器邏輯仍屬於另一個專案的範疇。

**目標**：使用者計畫證明 GAN 防禦比單純加噪訊/干擾**效率更好、成本更低**。實驗設計是：固定同一個「成本預算」（PSR/L2/Linf），比較不同防禦手法在同樣成本下能把攻擊者的 GE/N_TGE 推多遠。

### C.1 補齊 `perturbation.py`（PSR/L2/Linf）+ 貫通 Stage B 介面

`src/metrics/perturbation.py` 原本三個函式都是 stub，現在實作：

- `psr(clean, perturbed)`：`||perturbed-clean||₂ / ||clean||₂`，無量綱、跟訊號絕對尺度無關，是比較不同防禦手法「相對成本」的主要指標。
- `l2(clean, perturbed)`：擾動的絕對L2範數。
- `linf(clean, perturbed)`：單一時間點最大偏差，抓「平均便宜但某幾點爆量」的防禦。

三者都回傳 `(N,)` 逐軌跡陣列，不是單一純量——防禦成本的分佈（例如是否有肥尾）跟平均值一樣重要。新增 `tests/test_perturbation.py`（6個測試，含「相同L2成本、訊號音量不同時PSR排序會反過來」這種容易搞錯的案例），`tests/` 現在 48 個測試全過。

**Stage B 介面貫通**：`02_run_attack.py`／`03_evaluate.py` 原本的 docstring 就寫著「換一組防禦後波形重跑即可」，但實際上兩個腳本都寫死讀寫 `run_dir` 底下的 `probs.npy`／`metrics.json`，沒有真的留任何 CLI 掛勾——這次補上：

- `02_run_attack.py` 新增 `--traces PATH`（指向替代的原始 E 集 `.npy`，shape 需跟 `attack_traces[split.e]` 一致）+ `--out PATH`（**強制搭配使用，不給就報錯**，避免防禦評估不小心覆寫掉乾淨基準的 `probs.npy`）。
- `03_evaluate.py` 同理新增 `--probs PATH` + `--out PATH`。

**新增 `scripts/05_apply_defense.py`**：吃一個已訓練好的攻擊者 `--run`（拿它的 `data.path`／`split_indices.npz`），對 E 集套用指定防禦（目前只有 `gaussian`，重用 E02 訓練時用的同一個 `gaussian_augment` 純函式，這次是套在**已訓練完的原始 E 集軌跡上模擬部署時的雜訊防禦**，不是訓練期增強），輸出 `defended_traces.npy` + `cost_metrics.json`（PSR/L2/Linf 的 mean/median/p90/max），並印出後續要接 `02_run_attack.py`/`03_evaluate.py` 的指令。輸出目錄 `defenses/`，`.gitignore` 已排除（跟 `runs/` 同待遇）。

### C.2 第一組結果：攻擊者模型的選擇本身就是實驗設計的關鍵變數

用 `sigma_ratio=0.5` 的高斯噪訊防禦（PSR均值0.0446）分別打在兩個已訓練好的攻擊者上：

| 攻擊者 | 乾淨 N_TGE | 防禦後 N_TGE | 退化倍數 |
|---|---|---|---|
| E02（訓練時就用同款高斯噪訊增強過） | 206 | 230 | 1.1× |
| **E01（clean baseline，沒見過任何噪訊）** | 475 | **2276** | **4.8×** |

**同一個防禦、同一個成本，對不同攻擊者的效果差了 4 倍以上**——E02 因為訓練時就吃過同分佈的噪訊增強，對這款高斯防禦有內建的免疫力，拿它當「受害攻擊者」會系統性低估防禦效果。**這個發現直接決定了後續 GAN vs 噪訊比較的實驗設計**：要公平比較不同防禦手法，必須固定用同一個、沒有特別對任一防禦手法免疫過的攻擊者當基準（目前看來 E01 是比 E02 更中性的選擇），否則不同防禦手法的比較結果會受到「攻擊者剛好對哪種擾動免疫」這個混淆變數污染。

**下一步**（留待防禦端專案，非本次重構範疇）：以 E01 為基準攻擊者，掃幾個不同 `sigma_ratio` 建立完整的「高斯噪訊 PSR-vs-N_TGE」曲線，作為 GAN 防禦的比較基準線；desync/時域干擾防禦也可以用同一套 `05_apply_defense.py` 骨架加一個新的 `--defense` 分支接上去。

### C.3 高斯噪訊防禦基準曲線（固定E01為攻擊者）

用 `05_apply_defense.py` 掃 8 個 `sigma_ratio` 值（0.1–3.0），全部固定打在 E01（clean baseline，未對任何防禦免疫）上，這是 GAN 防禦之後要打敗的第一條基準線：

| sigma_ratio | PSR（成本） | L2均值 | N_TGE | GE@9000 | SR1@9000 | PI |
|---|---|---|---|---|---|---|
| 0.1 | 0.0089 | 6.66 | 514 | 0.00 | 1.00 | -0.618 |
| 0.25 | 0.0223 | 16.64 | 775 | 0.00 | 1.00 | -1.199 |
| 0.5 | 0.0446 | 33.28 | 2276 | 0.00 | 1.00 | -3.389 |
| 0.75 | 0.0668 | 49.93 | 5080 | 0.02 | 0.98 | -6.588 |
| 1.0 | 0.0891 | 66.57 | 7019 | 0.15 | 0.88 | -10.233 |
| **1.5** | **0.1337** | 99.85 | **None** | **1.81** | 0.22 | -17.825 |
| 2.0 | 0.1783 | 133.14 | None | 5.61 | 0.00 | -25.233 |
| 3.0 | 0.2674 | 199.71 | None | 43.68 | 0.00 | -38.660 |
...
## resnet（E07）訓練穩健性調查（防禦端附錄S轉來的上游問題）

### B.56 根因找到了：resnet 的 one-cycle 排程從來沒有跑完過，存下的 checkpoint 是 epoch 5 的近初始化模型

**問題來源**：防禦端 `dlsca-defense-v2` 附錄 S 用 resnet 做 D04 防禦的架構廣度驗證時，4 個種子的**乾淨基準**（完全沒有任何防禦介入）GE 出現 51.35／44.35／49.93／**212.82** 的巨大變異，最後一個種子甚至高於隨機基準 127.5（等於完全沒訓練起來）。防禦端當時正確地把根因往上游定位到「resnet 這組訓練配方本身穩健性不足」，並建議由攻擊端 repo 獨立確認。這節就是那個確認。

**先發現一個組態漂移（本身就是 bug）**：B.45-B.49 調出來的 resnet 配方（`max_lr=5e-5, end_percentage=0.1, scale_percentage=0.1, epochs=100`）**從來沒有寫回 `configs/model/resnet.yaml`**，那個檔案一直停在未調參的 `flat Adam lr=1e-3`，調過的值只活在命令列的 `--override` 旗標裡。也就是說，任何人照著 `--config configs/exp/E07_resnet.yaml` 跑一次，拿到的是**調參前**的基準，不是 B.49 記載的結果——直接違反 §2 P1「組態驅動，程式碼零修改」。已修正，並順手把 `configs/exp/E07_resnet.yaml` 的 `attack.max_traces` 從 1000 改成 9000（B.45-B.49 跟防禦端附錄 O/R/S 實際引用的每一個 E07 數字都是 9000 窗口下量的，留著 1000 會讓 config 跟所有已發表數字不一致）。

**接著追 4 個既有種子的 `train_history.csv`，根因立刻浮現**：

| 種子 | 訓練到第幾 epoch | 最佳 checkpoint 在 | 該 epoch 的 GE 預覽 | 正式 GE@9000 | 最終 train acc | 最終 loss |
|---|---|---|---|---|---|---|
| 42 | 50 | **epoch 20** | 89.10 | 51.35 | 0.68% | 5.5329 |
| 123 | 35 | **epoch 5** | 89.90 | 44.35 | 0.59% | 5.5367 |
| 777 | 35 | **epoch 5** | 114.80 | 49.93 | 0.56% | 5.5374 |
| 456 | 35 | **epoch 5** | 139.35 | 212.82 | 0.60% | 5.5376 |

三個關鍵事實：

1. **4 個種子沒有一個跑完 100 epochs**，全部在 epoch 35 或 50 被 `patience=6`（配 `eval_every=5`，即 30 epochs 無改善）早停。
2. **4 個種子裡有 3 個的最佳 checkpoint 就是 epoch 5——第一次評估**，之後 GE 預覽一路單調惡化，訓練從第一次評估之後就沒有再進步過。
3. **4 個種子的最終 train accuracy（0.56%–0.68%）跟 loss（5.5329–5.5376）幾乎完全一樣**，隨機基準是 accuracy 0.39%、loss log(256)=5.545——**包含「失敗」的 seed=456 在內，4 個種子全部都只比隨機基準好一點點，訓練指標本身完全分辨不出誰成功誰失敗**。

把 one-cycle 的實際 LR 軌跡算出來（`max_lr=5e-5, end_percentage=0.1, scale_percentage=0.1, epochs=100`，A=30000、batch=128）之後，整件事就完全對上了：

```
epoch   1 -> LR 6.0e-06        epoch  45 -> LR 5.0e-05  ← 排程峰值
epoch   5 -> LR 1.0e-05  ← 存下的checkpoint在這   epoch  50 -> LR 4.5e-05
epoch  10 -> LR 1.5e-05        epoch  90 -> LR 5.0e-06
epoch  20 -> LR 2.5e-05        epoch  95 -> LR 2.5e-06
epoch  35 -> LR 4.0e-05  ← 訓練在這裡被早停砍斷   epoch 100 -> LR 5.0e-08  ← 退火終點
```

**排程的峰值在 epoch 45，但訓練在 epoch 35 就被砍斷了——峰值從來沒到達過，峰值之後的整段下降期與退火期（epoch 45→100，LR 從 5e-5 一路退火到 5e-8）在 4 個種子裡一次都沒有執行過。** 而真正被存下來當作「最佳模型」的，是 epoch 5 的 checkpoint，那時 LR 才 1.0e-5，排程連上升段的五分之一都還沒走完。

**這代表 B.45-B.49 那輪四維度掃描的解讀需要修正**：`end_percentage`（控制退火期佔比）跟 `scale_percentage`（控制退火底部 LR 多低）這兩個維度，調的是**訓練從未抵達的那段曲線**；`epochs=100` 調的是一個從未跑滿的預算。當時掃出來的「局部最優」在數學上仍然是那些設定下的實測結果、數字沒有錯，但**它們之所以有效，機制跟當初以為的不一樣**——例如 B.46-B.47 觀察到「max_lr 越小越好，單調下降到 5e-5 才反轉」，在早停於 epoch 35 的前提下，真正的效果是「把唯一真的跑到的那前幾個 epoch 的有效 LR 壓低」，而不是「找到了適合這個架構的 one-cycle 峰值」。

**這也直接解釋了防禦端附錄 S 的種子不穩定**：被選出來的模型本質上是一個訓練了 5 個 epoch、幾乎還在初始化附近的網路，它的 GE 主要由初始化的運氣決定，不是由學習決定——所以 4 個種子會給出 44／50／51／213 這種橫跨 4 倍以上的散佈，也所以「D04 對 resnet 的效果」這個訊號會被完全淹沒。附錄 S 判斷「問題在更上游」是對的，這裡把上游的確切位置指出來了：**不是 resnet 這個架構不行，是 `patience=6` 這個沿用自 flat-LR 時代的早停設定，跟 one-cycle 排程根本不相容**（`configs/model/cnn_light.yaml` 早就因為同一個理由把 patience 設到 100 並寫了註解說明，resnet 這邊漏了）。

### B.57 Phase 1（補到 10 個種子）：失敗率是 50%，不是附錄 S 估計的 25%；而且我上一節的根因假設被自己的資料推翻了一半

補 6 個新種子（1、7、99、555、2024、31337），連同附錄 S 既有的 4 個，共 **10 個種子**，全部用 B.56 修好的 config（完全相同的配方，唯一變因是 `seed`），全部走 100-run 正式評估：

| 種子 | 訓練到第幾 epoch | 最佳 checkpoint | 訓練期 20-run 預覽 GE | **正式 GE@9000** |
|---|---|---|---|---|
| 456 | 35 | ep5 | 139.35 | **212.82** |
| 7 | **100（唯一跑完全程）** | ep90 | **65.35（10個裡最好）** | **212.48** |
| 1 | 50 | ep20 | 90.10 | 185.53 |
| 555 | 75 | ep45 | 97.70 | 184.14 |
| 31337 | 35 | ep5 | 108.10 | 135.37 |
| 99 | 50 | ep20 | 65.60 | 95.71 |
| 42 | 50 | ep20 | 89.10 | 51.35 |
| 777 | 35 | ep5 | 114.80 | 49.93 |
| 2024 | 35 | ep5 | 78.95 | 47.49 |
| 123 | 35 | ep5 | 89.90 | 44.35 |

```
n=10   min=44.35  max=212.82  median=115.54  mean=121.92  標準差=72.24
跑完 100 epochs 的：1/10
GE@9000 >= 隨機基準 127.5（等於完全沒有可用的攻擊能力）：5/10
```

**結論一（直接回答防禦端附錄 S 提出的問題）**：resnet 這組訓練配方的失敗率是 **10 個種子裡 5 個（50%）**，不是附錄 S 從 4 個種子估計的 25%——樣本數翻倍之後，問題比原本以為的嚴重一倍。更關鍵的是 **10 個種子的平均 GE@9000 是 121.92，隨機基準是 127.5**：平均而言，這個配方訓練出來的 resnet 攻擊者幾乎跟亂猜一樣。附錄 S 說的「resnet 訓練配方本身穩健性不足」不只成立，而且程度上是「這個配方沒有可用的攻擊能力，偶爾靠運氣撈到一次好結果」。

**結論二（我在 B.56 提的根因假設，被這批資料推翻了一半——必須誠實記錄）**：B.56 從 4 個種子推論出「`patience=6` 砍掉了 one-cycle 排程，所以模型沒訓練好」，並預期「讓排程跑完就會好」。這批資料裡 **seed=7 正是唯一跑完 100 epochs 的種子，訓練曲線是教科書等級的單調下降（預覽 163→156→144→129→109→94→84→77→73→70→…→65），訓練期預覽 65.35 是 10 個種子裡最好的一個——但它的正式 GE@9000 是 212.48，10 個裡倒數第二**。

也就是說：**B.56 描述的機制（排程沒跑完、checkpoint 停在 epoch 5）在事實層面完全正確，但它不足以解釋正式 GE 的變異**。「讓排程跑完」這件事，在唯一觀察到的樣本上不但沒有帶來改善，結果還特別差。

**結論三（比前兩點更值得寫進報告的一個量化發現）**：把 10 個種子的「訓練期 20-run 預覽 GE」跟「正式 100-run GE@9000」做相關性分析：
...
**兩個相關係數都接近 0，在 n=10 下跟「毫無關係」無法區分**。這代表在 resnet 上，`GEModelSelection` 賴以選 checkpoint 的那個訓練期預覽指標，**跟真正的攻擊效能基本上沒有關聯**——選模機制實際上是在近乎隨機地挑 checkpoint。這比「早停設定不對」是更根本的問題：即使把排程跑完（seed=7），選模機制依然會從 100 個 epoch 裡挑出一個正式評估表現很差的 checkpoint。

這也讓本專案一路反覆遇到的「訓練期預覽會騙人」（附錄 B.24 resnet、B.28/B.35 desync、B.50 cnn_best，防禦端附錄 T/U）從一個定性的警告，變成一個有數字的定量結論：**至少在 resnet 上，這個預覽的預測力就是 0**。`n_runs_val=20`＋1000 條軌跡的快速預覽，抽樣雜訊大到蓋過了真實訊號。

**下一步**：B.56 的 `patience` 假設雖然在 seed=7 這個單一樣本上沒有兌現，但 n=1 不足以下結論（這正是本專案一路在強調的事，不能因為結論方向不合自己的假設就用單一樣本草率收尾）。Phase 2 用同樣的 10 個種子、只把 `patience` 從 6 改成 100（強制跑完排程），做一次完整的對照，見 B.58。

### B.58 Phase 2（patience=100，同樣 10 個種子）：假設被完全推翻——排程跑完之後，選出來的 checkpoint 跟結果一模一樣

把 `train.selection.patience` 從 6 改成 100（強制跑滿 100 epochs、讓 one-cycle 的峰值與退火期完整執行），其餘完全不動，同樣 10 個種子重跑一次：

| 種子 | patience=6：epochs / 最佳ep / GE@9000 | patience=100：epochs / 最佳ep / GE@9000 |
|---|---|---|
| 7 | 100 / 90 / 212.48 | 100 / 90 / **212.94** |
| 456 | 35 / 5 / 212.82 | 100 / 5 / **212.84** |
| 1 | 50 / 20 / 185.53 | 100 / 20 / **185.40** |
| 555 | 75 / 45 / 184.14 | 100 / 45 / **183.44** |
| 31337 | 35 / 5 / 135.37 | 100 / 5 / **135.36** |
| 99 | 50 / 20 / 95.71 | 100 / 20 / **95.81** |
| 42 | 50 / 20 / 51.35 | 100 / 20 / **51.30** |
| 777 | 35 / 5 / 49.93 | 100 / 5 / **49.90** |
| 2024 | 35 / 5 / 47.49 | 100 / 5 / **47.48** |
| 123 | 35 / 5 / 44.35 | 100 / 5 / **44.24** |

```
patience=6    n=10 min=44.35 max=212.82 median=115.54 mean=121.92 sd=72.24 失敗5/10
patience=100  n=10 min=44.24 max=212.94 median=115.59 mean=121.87 sd=72.25 失敗5/10
配對平均差（p100 − p6）= −0.05
跑滿 100 epochs：patience=6 是 1/10，patience=100 是 10/10
```

**結果乾淨到沒有任何模糊空間：10 個種子裡，每一個種子選出來的最佳 checkpoint epoch 完全相同（90/5/20/45/5/20/20/5/5/5，跟 Phase 1 逐一對應一致），正式 GE@9000 的差異全部在 ±0.1 以內（那是重跑推論的浮點雜訊，不是真實差異），平均值、標準差、失敗率三個統計量全部原封不動。**

**這代表 B.56 提出、B.57 已經開始動搖的那個假設，現在被 n=10 的完整對照徹底推翻**：`patience=6` **並沒有**砍掉任何有用的訓練。強制多跑 25–65 個 epoch（含 one-cycle 的峰值與整段退火期）之後，那些新增的 epoch **沒有任何一個**產生比原本已經找到的最佳 checkpoint 更好的預覽 GE，所以選模機制選出了完全相同的 checkpoint。早停不是問題的原因，早停只是**忠實反映了「訓練確實在很早就停止進步」這個事實**。

**修正後的正確結論**：問題不在早停設定、也不在 one-cycle 排程沒跑完，而在更根本的地方——**resnet 配上這組配方，本質上就沒有在學習**。三條證據互相印證：

1. **訓練指標本身**：10 個種子的最終 train accuracy 全部落在 0.56%–0.68%（隨機基準 0.39%），loss 全部落在 5.53–5.54（隨機基準 log(256)=5.545）——**不管跑 35 epoch 還是 100 epoch，模型都只比隨機基準好一點點**。
2. **多跑無效**：Phase 2 證實把訓練時間拉長 2–3 倍，結果完全不變。
3. **選模指標無預測力**：訓練期 20-run 預覽 vs 正式 100-run GE@9000 的相關性，Phase 1 是 Pearson r=0.186，Phase 2 是 r=0.185——兩次獨立測量給出幾乎相同的、接近零的相關係數。

**因此 B.45-B.49 那輪四維度掃描的定位需要修正**：那四個維度（max_lr、end_percentage、scale_percentage、epochs）掃出來的「局部最優」，是在一個平均表現等同隨機猜測（10種子平均 GE=121.92 vs 隨機基準 127.5）、且選模指標本身沒有預測力的系統上量出來的——**當時看到的維度間差異，很可能主要是種子雜訊，不是超參數的真實效果**。B.49 記錄的 GE@9000=51.32 這個數字本身沒有錯（它是那次特定執行的真實測量值），但它落在本節量到的 44.35–212.82 這個分佈的下緣，屬於分佈裡運氣好的那一端，**不應該被當成「resnet 這組配方的代表性效能」引用**。

**對防禦端 `dlsca-defense-v2` 的直接影響**：附錄 O／R／S 建立在 resnet 之上的每一個結論（D04 是否可遷移到異架構）——不只是「種子數不足」，而是**底層的攻擊者有 50% 機率根本不存在攻擊能力，且即使成功的那 50% 也是初始化運氣的產物**。附錄 S 判定「受阻、不建議繼續加種子」是正確的處置；這裡確認了受阻的確切原因，並且確認**在 resnet 的訓練配方被真正修好之前（不是調 patience 這種等級的修改，是需要重新檢視架構或訓練方法論），這條架構廣度驗證線不具備可用的基礎**。

**方法論價值（值得寫進報告）**：這兩節本身是一個完整的「提出假設 → 設計對照 → 假設被自己的資料推翻 → 修正結論」的案例。B.56 從 4 個種子推論出一個機制上完全正確的觀察（排程確實沒跑完、checkpoint 確實停在 epoch 5、LR 確實只走到峰值的五分之一），並據此提出一個看似理所當然的因果假設；B.57 的 seed=7 提供了第一個反例；B.58 用 n=10 的完整對照確認反例不是特例。**「機制描述正確」跟「因果假設正確」是兩件事**——排程沒跑完是事實，但它不是失敗的原因，而是失敗的另一個症狀。

### B.59 決定性對照：訓練 1 個 epoch 的 resnet，跟完整跑 35–100 epochs 的「調過參」版本統計上無法區分

B.58 證實了「多跑沒用」，但還留下一個更根本的問題沒回答：**這 35–100 個 epoch 的訓練，到底有沒有貢獻任何東西？** 設計一個直接的對照——同樣 10 個種子，`train.epochs=1`（配 `eval_every=1` 讓選模機制看得到第 1 個 epoch），其餘完全相同，一樣走 100-run 正式評估：

| 種子 | **1 epoch** GE@9000 | SR1 | 完整訓練 GE@9000 | 哪個好 |
|---|---|---|---|---|
| 42 | **2.27** | **0.25** | 51.35 | **1 epoch** |
| 555 | **19.27** | 0.00 | 184.14 | **1 epoch** |
| 99 | 53.77 | 0.00 | 95.71 | **1 epoch** |
| 123 | 60.78 | 0.00 | 44.35 | 完整 |
| 2024 | 79.33 | 0.00 | 47.49 | 完整 |
| 777 | 126.00 | 0.00 | 49.93 | 完整 |
| 1 | 134.74 | 0.00 | 185.53 | **1 epoch** |
| 31337 | 149.07 | 0.00 | 135.37 | 完整 |
| 7 | 160.35 | 0.00 | 212.48 | **1 epoch** |
| 456 | 232.98 | 0.00 | 212.82 | 完整 |

```
1 epoch    ： mean=101.86  median=102.66  sd=71.30  失敗(>=127.5)=4/10
完整訓練   ： mean=121.92  median=115.54  sd=72.24  失敗(>=127.5)=5/10
1 epoch 勝出：5/10（完全是擲硬幣）

配對差（1epoch − 完整）：mean=−20.06, sd=66.91
配對 t = −0.948（df=9，要 |t|>2.26 才有 p<0.05）
Wilcoxon W+=19 / W−=36（n=10 要 W<=8 才有 p<0.05，兩邊都差很遠）
```

**結論：訓練 1 個 epoch 跟完整跑 35–100 epochs，在統計上完全無法區分**（配對 t 檢定與 Wilcoxon 符號秩檢定都遠遠不顯著）。1-epoch 版本的平均值名目上甚至比較好（101.86 vs 121.92），但在 sd≈67 的配對差異下，這個差距沒有任何統計意義——正確的解讀不是「1 epoch 比較好」，而是**「訓練與否對結果沒有可測量的影響」**。

**兩個補強證據**：

1. **變異幅度幾乎完全相同**：sd(1 epoch)=71.30 vs sd(完整訓練)=72.24。**如果訓練有在做事，它至少應該改變結果的分佈形狀；實際上分佈的離散程度一模一樣**——這強烈指向「GE 的變異幾乎全部來自權重初始化，訓練只是在初始化附近做無方向的漂移」。
2. **1-epoch 模型的 train accuracy 是 0.0038–0.0040，也就是精確落在隨機基準 1/256=0.0039 上**——這些模型在自己的訓練集上完全沒有分類能力，但其中 seed=42 卻能在 9000 條軌跡上把 GE 壓到 2.27、100 次獨立攻擊有 25 次成功。已逐項驗證這不是評估管線的假象：`probs` 每列和為 1、抽樣 100 條軌跡預測全部互不相同（非常數輸出崩潰）、GE 曲線平滑單調下降（107.77→78.01→69.52→37.48→2.27）。真正的解釋是**這個模型的預測極度平坦（每列最大機率只有 0.0073，均勻分佈是 0.0039），帶著非常微弱但方向正確的偏好，在 9000 條軌跡的對數累加下積成一個真實的攻擊**。

**這條對照把整件事定案了**：resnet 這組配方之所以不穩定，不是早停砍太早（B.58 已推翻）、不是排程沒調好、也不是種子數不夠——是**訓練本身對最終攻擊效能沒有可測量的貢獻，全部的結果都由初始化的運氣決定**。附錄 B.45-B.49 那輪四維度掃描（約 14–15 次訓練）當時看到的所有維度間差異，事後看幾乎確定是在量測這個 sd≈72 的初始化雜訊，不是超參數的效果——這也解釋了為什麼那輪掃描的每一個維度都「剛好」在預設值附近找到局部最優（`end_percentage=0.1`、`scale_percentage=0.1`、`epochs=100` 三個贏家有兩個就是預設值）：在一個沒有真實訊號的系統上做網格搜尋，贏家的位置本來就是隨機的。

**給後續的建議（明確不在本次範圍內執行）**：resnet 要成為一個可用的對照架構，需要的不是繼續調這四個超參數，而是回到更上游——先確認 `src/models/resnet.py` 這個 28,816 參數的架構配上 Adam＋這個資料規模，梯度到底有沒有在往有意義的方向走（例如檢查 train loss 是否真的能被推離 log(256)、或先在遠小的子問題上驗證架構本身可學）。在那之前，**任何建立在 resnet 之上的比較結論（包含防禦端附錄 O／R／S 的 D04 架構遷移性）都缺乏可用的基礎**。

### B.60 resnet 訓練穩健性調查收尾總表

| 階段 | 做了什麼 | 種子數 | 結論 |
|---|---|---|---|
| 起點（防禦端附錄 S） | 4 種子觀察到乾淨基準 GE 變異 44–213 | 4 | 判定「受阻」，根因往上游定位到攻擊端配方 |
| 修 config 漂移（B.56） | 把 B.45-B.49 調出的配方寫回 `configs/model/resnet.yaml`；`max_traces` 1000→9000 | — | 修掉一個真的可重現性 bug（照 config 跑會拿到調參前的基準） |
| Phase 1（B.57） | 補到 10 種子，全部 100-run 正式評估 | 10 | **失敗率 5/10（非 25%）**；10 種子平均 GE=121.92 ≈ 隨機基準 127.5；**預覽 vs 正式相關性 r=0.186（無預測力）** |
| Phase 2（B.58） | `patience` 6→100，強制跑滿排程 | 10 | **假設推翻**：10 個種子選出的 checkpoint epoch 完全相同、GE 差異全在 ±0.1 內、平均/標準差/失敗率原封不動 |
| 對照（B.59） | `epochs=1`，同樣 10 種子 | 10 | **訓練 1 epoch 與完整訓練統計上無法區分**（配對 t=−0.948、Wilcoxon 不顯著；sd 71.30 vs 72.24） |

**最終定論**：resnet（E07）目前這組配方**沒有可用的攻擊能力**，結果完全由權重初始化決定，訓練過程沒有可測量的貢獻。B.49 記錄的 `GE@9000=51.32` 是這個 sd≈72 分佈裡運氣好的一個抽樣，不具代表性，**不應被引用為「resnet 的效能」**。§8.2 的 E07 在跨實驗比較表裡應標註為「配方無效，結果為初始化雜訊」，而非與 E01／E02／E05／E08 並列比較。

## 資料集廣度：ASCADv1 variable-key（防禦端未來工作項目2）

### B.61 引入 variable-key 資料庫：四個結構差異，全部實測確認而非假設

**動機**：到目前為止本專案（含防禦端子專案）的每一個結論，都建立在**單一資料庫**——ASCAD fixed-key desync0——之上。variable-key 是 SCA 文獻標準的下一步，因為它拿掉了 fixed-key 設定裡最寬鬆的假設：profiling 只有一把金鑰時，模型可以部分「背下」那把金鑰的 S-box 那一列；換成 256 把金鑰後，模型被迫學一個**與金鑰無關的洩漏函數**。

`data/ascad-variable.h5`（419MB，sha256 `d834da6c…`，與上游 Readme 一致）。跟 fixed-key 的四個結構差異，逐項實測確認：

| | fixed-key `ASCAD.h5` | **variable-key `ascad-variable.h5`** |
|---|---|---|
| Profiling / Attack | 50,000 / 10,000 | **200,000 / 100,000** |
| trace_len | 700 | **1400** |
| masks 欄數 | 16 | **18** |
| Profiling 金鑰 | 固定 | **16 個 byte 全部各有 256 種值**（byte2 分佈 0.00353–0.00426，均勻值 0.00391，即抽取出來的 profiling 集是純隨機金鑰，沒有 Readme 提到的「三分之一固定金鑰」子集） |
| Attack 金鑰 | 固定 | 固定（`key[2]=34`，16 個 byte 全部固定） |
| profiling 中與攻擊金鑰同 byte2 的比例 | — | **0.39%（≈1/256）**，等於無法靠記憶目標金鑰取巧 |

**mask 欄位的索引方式不同，這點特別容易踩坑**。掃描全部欄位後發現，兩個資料庫都**各有恰好兩欄帶真實訊號**，其餘欄位一律落在 ~0.011 的雜訊地板：

| | 目標 byte 的 mask | `r_out`（共用輸出遮罩） | 其餘欄位 |
|---|---|---|---|
| fixed-key（16欄） | **col0** = 6.3003 @pt517 | col15 = 0.7671 @pt492 | ~0.011 |
| variable-key（18欄） | **col2** = 1.5641 @pt1071 | col17 = 1.4391 @pt1000 | ~0.011 |

差異的原因也查清楚了：**variable-key 的 col0/col1 是常數（uniq=1）**——ASCAD v1 的 byte 0、1 不做遮罩，所以 variable-key 直接用 byte 編號當索引（`masks[i]` = byte i 的 mask），而 fixed-key 把 byte 2–15 存在索引 0–13。因此**這裡的 mask_index 必須是 2，不是 0**。已在 `configs/exp/E09_ascad_variable.yaml` 明確釘死（不靠自動偵測），避免不同執行間悄悄漂移。

**`00_inspect_data.py` 通過**：masked-label SNR 峰值 1.5641 @ point 1071 vs 未遮罩對照組 0.0114（**137倍**），且只有 5/1400 個點超過峰值一半（訊號集中），desync 欄位全為 0。

**順手修掉自動偵測的一個真缺陷**：原本的警告條件是「冠軍沒有贏亞軍兩倍就警告」，但如上表所示，**ASCAD 本來就有兩欄帶真實訊號**（目標 byte 的 mask 與 r_out），所以這個判準在健康的資料庫上會誤報（variable-key：1.5641 vs 1.4391，只差 1.09 倍）。正確的問題是「冠軍有沒有明顯高於**雜訊地板**」，已改為跟所有欄位的**中位數**比較（穩健：最多只有兩欄帶訊號，中位數必定落在雜訊欄）。三個資料庫實測驗證：`ASCAD.h5` 與 `ascad-variable.h5` 改印一則說明性的 note，`ASCAD_desync50.h5` **仍然正確發出警告**（所有欄位一起塌到地板，冠軍 0.0119 vs 中位數 0.0100）。

**另外補上 `data.trace_len` 與實際資料的一致性檢查**：這個值只餵給模型的 `input_dim`，先前沒有任何地方驗證它跟檔案相符，寫錯只會在 `fit()` 深處炸出一個難讀的 Keras shape 錯誤——而在資料庫不再共用同一個 trace 長度之後，這變成很容易犯的錯。

**`cnn_light` 的參數量會隨視窗變化**（flatten→FC 層與輸入長度成正比）：trace_len=700 時 18,640，trace_len=1400 時 **32,640**。這是架構與抽取視窗共同決定的，無法避免，但必須誠實記為「E09 與 E01 並非字面上同一個模型」的差異，不能藏起來。

### B.62 抓到一個會讓 variable-key 選模完全失效的 bug：`GEModelSelection` 假設 V 只有一把金鑰

跑完 E09 之後檢查選模邏輯，發現一個**沉默的正確性 bug**：

`src/train/callbacks.py::GEModelSelection.__init__` 用 `get_correct_key(meta_val, target_byte)` 取得「正確金鑰」，而該函式回傳的是 `meta_val["key"][0][target_byte]`——**驗證集第 0 條軌跡的金鑰**。金鑰恢復的數學前提是「所有軌跡共用同一把金鑰」（分數要跨軌跡累加），這在 fixed-key 上成立，在 variable-key 上完全不成立。實測 E09 的 V：

```
V 大小：5000
V 中 key[2] 的相異值：256
get_correct_key(V) 回傳：189（只是第 0 條的金鑰）
V 中真正持有這把金鑰的比例：0.54%
```

**也就是說 E09 訓練期間印出的每一個 GE 預覽，都是拿 99.46% 條軌跡去對一把錯誤的金鑰假設算出來的，數值本身沒有意義，`GEModelSelection` 的 checkpoint 選擇因此等同隨機挑選。**

**影響範圍必須講清楚**：
- **最終評估不受影響**——攻擊集的金鑰是固定的（實測 E 的 `key[2]` 相異值 = 1），所以 `03_evaluate.py` 算出的 `GE@9000=84.76` 是一個真實、有效的測量值；只是它測的是一個「被隨機選出來的 checkpoint」。
- **所有 fixed-key 實驗（E01–E08、全部 desync 系列、防禦端全部配方）完全不受影響**，因為那些資料庫的 V 本來就只有一把金鑰，`get_correct_key` 的行為是正確的。

**修法**：在 `__init__` 偵測 `meta_val["key"][:, target_byte]` 是否為常數。
- **常數（fixed-key）→ 完全走原本的 GE 路徑，行為逐位元不變**（這點很重要，不能擾動任何既有結果）。
- **非常數（variable-key）→ GE 在 V 上根本無法計算**，改用一個與金鑰無關的替代判準：**每條軌跡「自己的真實類別」的平均排名**（用該條軌跡自己已知的金鑰算標籤）。沿用 keyrank 的「嚴格領先者個數」慣例，尺度一致（0 = 完美，256 類的隨機基準 ≈127.5）。它衡量的正是驅動 GE 的那個量——單條軌跡上的判別力——但不需要所有軌跡共用金鑰。若此時沒有提供 `y_val`，直接 `raise ValueError`，**不允許沉默地退回錯誤行為**。

log 的標籤也跟著改（`final_GE=` vs `mean_true_rank=`），避免有人事後把兩個不同的量當成同一個讀。

新增 `tests/test_callbacks.py`（8 個測試）：fixed-key 仍走 GE 路徑且金鑰讀取正確、fixed-key 不需要 `y_val`（既有呼叫端不受影響）、variable-key 會被偵測到、variable-key 缺 `y_val` 會拋錯而非誤算、替代判準對完美模型為 0、對隨機輸出落在 127.5 附近、能正確把較強的模型排在較低分、以及一個直接記錄「為什麼需要這個 fallback」的測試（用單一金鑰算出來的標籤，只有 ~1/256 與逐軌跡真實標籤相符）。`tests/` 現在 **60 個測試全過**。

### B.63 GPU 訓練根本不可重現——§11 的驗收標準在 GPU server 上一直是不成立的（已修）

**怎麼發現的**：B.62 修完 `GEModelSelection` 之後，想確認「fixed-key 的行為逐位元沒被動到」，於是重跑 E01 跟已存的 `runs/E01_baseline_clean_20260816_1302/` 比對。結果對不上。**但連 train loss / accuracy 都對不上**——而我改的是 callback，它只做推論與存檔，碰不到梯度，邏輯上不可能改變 loss。

第一個解釋找到了：那個已存的 E01 是 **8/16 13:02** 跑的，而 GPU server 是 B.27 才建置（8/16 18:01 之後）。`env.json` 確認：舊 run 是 `"gpus": []`（純 CPU），新 run 是 `GPU:0`。**CPU 與 GPU 的浮點運算順序不同，訓練軌跡本來就會分岔——我這個回歸測試從設計上就是無效的**（拿 CPU 時代的結果去比 GPU 重跑）。

**但接著做的對照實驗挖出一個更嚴重的問題**。既然不能跟 CPU 舊結果比，就改比「同一台 GPU 上、同一份 config、同一個 seed，跑兩次」：

```
run A: runs_regress/E01_baseline_clean_20260901_175501_1894600
run B: runs_regress/E01_baseline_clean_20260901_180137_1912951

GE previews identical   : False
loss/accuracy identical : False
  epoch1  loss 5.567685127258301  vs  5.5678815841674805
  epoch2  loss 5.54701042175293   vs  5.546788215637207
  epoch3  loss 5.543455123901367  vs  5.543044090270996
```

**同一份 config、同一個 seed，兩次執行從第 1 個 epoch 就開始分岔。** 這直接違反 CLAUDE.md §11 的第一條驗收標準：「任一實驗可由單一指令完整重跑，兩次執行的 `metrics.json` 完全一致」。

**根因**：`src/seeding.py::set_global_seed` 有把 python / numpy / tensorflow 三處 RNG 都設好（§8.1 的種子分層設計本身沒問題），但**設定 RNG 種子在 GPU 上不足以保證可重現**——cuDNN / cuBLAS 會在執行期自行挑選演算法，且歸約（reduction）用 atomics，累加順序每次都可能不同。這在 CPU 上不會發生，所以這個問題在 B.27 建 GPU server 之前一直不存在，建好之後就一直存在但沒被發現。

**修法**：`set_global_seed` 加上 `tf.config.experimental.enable_op_determinism()`（必須在建立任何 op 之前呼叫），並由 `configs/base.yaml` 的 `deterministic: true` 控制（P1 組態驅動；想換回速度的人可以關掉）。

**驗證**（同一份 config 連續跑兩次）：

```
run A: E01_baseline_clean_20260901_180834_1931504
run B: E01_baseline_clean_20260901_180945_1934104
train_history IDENTICAL: True
```

**代價**：約 9–11 秒/epoch，相對關閉時的 6–7 秒/epoch，慢約 40–50%。以換到 §11 要求的可重現性而言划算，且可用 config 關掉。

**對既有結論的影響，必須誠實交代**：

1. **所有既有的數字本身都仍然有效**——它們是真實執行的真實測量值，沒有一個需要作廢。
2. **但「同一個 config 重跑會得到同樣結果」這件事，在 8/16 之後所有 GPU 上跑的實驗都不成立**。這代表本專案一路記錄的種子敏感度（附錄 M/P/Q/R/S、以及 B.57-B.59 的 resnet 調查）裡，**有一部分變異其實來自 GPU 的執行期不確定性，不是全部來自「換了種子」**——因為即使種子固定，兩次執行也會不同。
3. **這不會推翻那些結論，反而讓它們更強**：原本的結論是「單一次執行的結果不可信、必須多次重複」。現在知道連「固定種子重跑」都不會一致，這個結論只會更成立，不會更弱。需要修正的是**歸因的精確度**——B.59 說 resnet 的結果「完全由權重初始化決定」，更精確的說法是「由初始化**與執行期數值雜訊**共同決定，總之不是由訓練決定」；1-epoch 對照的核心結論（訓練無貢獻）完全不受影響，因為對照的兩組受到的是同一種雜訊。
4. **未來若要精確拆分「種子變異」與「執行期變異」**，現在有了乾淨的工具：固定種子、開啟 `deterministic: true` 重跑數次即可量出純粹的執行期變異（理論上應為 0），再與跨種子變異對比。這是一個便宜且有價值的後續實驗，但不在本次範圍內。

**方法論價值**：這是本次調查中第二個「為了驗證 A，意外發現更上游的 B」的案例（第一個是 B.56-B.60：為了驗證 D04 的架構遷移性，發現 resnet 根本沒在訓練）。也再次印證一個模式——**回歸測試設計錯誤本身會產生假訊號**：如果沒有注意到「舊結果是 CPU 跑的」，就會誤以為 B.62 的 callback 修改破壞了 fixed-key 行為，進而去「修」一個根本不存在的 bug。

### B.64 E09 結果：E01 的配方在 variable-key 上完全學不起來，而且問題不是資料量——但鑑別診斷證實管線完全正確

**E09（variable-key，E01 配方原封不動，A=30000）**：

```
GE @ N=9000  = 115.18   （隨機基準 127.5）
SR1 @ N=9000 = 0.0000
N_TGE        = None
PI           = -0.1894 bits
```

對照 **E01（fixed-key，同一套配方）**：`N_TGE=475`、`GE@9000=0.00`、`SR1=1.00`。

**訓練期的 `mean_true_rank`（B.62 新增的、與金鑰無關的判別力指標，隨機基準同樣是 127.5）更能說明問題**：

```
epoch  5: 127.05   epoch 20: 127.31   epoch 35: 127.36
epoch 10: 127.42   epoch 25: 127.55   epoch 40: 127.89
epoch 15: 127.30   epoch 30: 127.62   epoch 45: 128.63   epoch 50: 128.34
```

**整整 50 個 epoch，單條軌跡上的判別力從頭到尾釘在隨機基準上，一次都沒有離開過。** 這不是「學得慢」，是**完全沒有學到任何逐軌跡的判別能力**——`GE@9000=115.18` 這個略低於隨機基準的數字，是把接近雜訊層級的微弱證據在 9000 條軌跡上累加出來的結果，不是模型真的會分辨。

（附帶一提：B.62 修好選模之前那次 E09 跑出 `GE@9000=84.76`，比修好之後的 115.18 還「好」。這不是修壞了——因為 `mean_true_rank` 在每個 epoch 都是隨機水準，**任何選模判準都只是在一堆同樣糟的 checkpoint 裡隨機挑一個**，兩個數字都是同一個「幾乎沒學到」分佈裡的抽樣。這本身就是「模型沒學到東西」的另一個佐證。）

**鑑別診斷（比照 B.18 對 desync50 的做法）：把最好學的目標搬過來測**。用同一個資料庫、同一套配方、同一個 A=30000，只把 leakage model 換成 `ID_MASKED`（遮罩已知，本專案在 fixed-key 上最容易的目標，E08 的 `N_TGE=3`）：

```
mean_true_rank: 28.34(ep5) -> 18.83(ep25) -> 15.93(ep35) -> 14.82(ep50，仍在改善)

N_TGE        = 3          （跟 fixed-key E08 的 N_TGE=3 完全一致）
N_SR90       = 4
GE @ N=9000  = 0.0000
SR1 @ N=9000 = 1.0000
PI           = 2.6557 bits
...
**三條軌跡就完全破解金鑰**。這一次對照把整條管線在這個資料庫上驗證乾淨了：**資料沒問題、`mask_index=2` 選對了、labels/前處理/評估/scores 全部正確**——因為只要把遮罩告訴模型，它就能把洩漏完美地抽出來。

**因此結論可以下得很明確**：失敗**不是**管線 bug、**不是** mask index 抄錯、**不是**資料下載錯，而是**「遮罩未知（ID）」這個任務本身在 variable-key 上，對 `cnn_light` 這個配方來說學不起來**。

**資料量也不是原因**（E09b，A=120000/V=20000/D=60000，用滿資料庫原生的 200,000 條 profiling，其餘完全相同）：

```
mean_true_rank: 126.88(ep5) 127.80 127.66 127.06 127.27 127.20 ... 126.87(ep40，全程最佳) 127.23 127.45(ep50)
...
**把訓練資料放大四倍（30000→120000），判別力依然釘死在隨機基準上**（最佳 126.87 vs E09 的 127.05，差距毫無意義），`N_TGE` 依然是 `None`，`SR1` 依然只有 3%。

**這裡有一個很容易寫錯、必須特別小心的地方**：E09b 的 `GE@9000=79.63` 看起來比 E09 的 115.18「好」，**但不能解讀成「資料放大四倍讓 GE 從 115 進步到 80」**。理由是本節前面已經說明的那件事——`mean_true_rank` 全程在隨機水準，代表所有 checkpoint 一樣糟，選模只是在隨機挑；同一個 E09 設定在修選模前後就分別給出 84.76 與 115.18 兩個值。**79.63 落在這個既有散佈範圍之內，不構成改善的證據**。三次執行（84.76 / 115.18 / 79.63）的共同點才是真正的結論：**全部 `N_TGE=None`、`SR1` 介於 0–0.03、`mean_true_rank` 全部在隨機基準——資料量翻四倍沒有帶來任何性質上的改變**。

**這個發現對整份報告最重要的意涵**：

**fixed-key ASCAD 的 ID 任務，比它看起來的樣子容易得多。** E01 的 `N_TGE=475` 不應該被解讀成「`cnn_light` 能攻破加了遮罩的 AES」，正確的解讀是「**`cnn_light` 能攻破加了遮罩的 AES——在 profiling 與 attack 共用同一把金鑰的前提下**」。profiling 只有一把金鑰時，`Z=Sbox[p⊕k]` 是明文的一個確定函數，模型可以利用這個結構抄捷徑；金鑰一變動，捷徑消失，剩下的是一個真正的二階問題（金鑰未知**且**遮罩未知），這個 18K–32K 參數的小模型做不到。

這跟 SCA 文獻對 ASCAD variable-key 的一般認知一致（variable-key 通常需要更大的模型與更多軌跡），但**在本專案的脈絡下，它是對 E01 這個「專案頭號成果」的一個實質的可推廣性限制**，而且是只有換資料庫才會現形的限制——正是防禦端未來工作項目 2 設計來檢驗的東西。

**對防禦端子專案的連帶意涵**：`dlsca-defense-v2` 的每一個結論（D01–D08、D04+jamming 縱深防禦、全部三種子驗證）都是在 fixed-key、以 E01 為基準攻擊者的前提下建立的。這裡的發現不會推翻那些結論（它們在自己的設定下都是誠實的測量），但**明確界定了它們的適用範圍**：那些防禦是針對「一個在共用金鑰前提下才成立的攻擊者」評估的。防禦在 variable-key 情境下是否同樣有效，目前**完全未知**，且無法從現有結果外推——因為在那個情境下，連未受防禦的基準攻擊者都還不存在。

### B.65 資料集廣度調查收尾總表（防禦端未來工作項目 2 的第一階段）

| 實驗 | 資料庫 | 洩漏模型 | A 大小 | mean_true_rank（隨機=127.5） | GE@9000 | SR1 | N_TGE |
|---|---|---|---|---|---|---|---|
| E01（既有基準） | fixed-key | ID | 30,000 | — | **0.00** | **1.00** | **475** |
| E08（既有基準） | fixed-key | ID_MASKED | 30,000 | — | 0.00 | 1.00 | **3** |
| **E09** | **variable-key** | ID | 30,000 | **127.05**（釘在隨機） | 115.18 | 0.00 | **None** |
| **E09b** | **variable-key** | ID | **120,000** | **126.87**（釘在隨機） | 79.63 | 0.03 | **None** |
| **E09 診斷** | **variable-key** | **ID_MASKED** | 30,000 | **14.82** | **0.0000** | **1.00** | **3** |

**四點結論**：

1. **管線在 variable-key 上完全正確**——`ID_MASKED` 診斷用 3 條軌跡破解金鑰，與 fixed-key 的 E08 完全一致（`N_TGE=3`）。資料、`mask_index=2`、labels、前處理、評估全部驗證過，失敗不是 bug。
2. **E01 的配方在 variable-key 的 ID 任務上完全學不起來**——50 個 epoch 內判別力從未離開隨機基準。
3. **不是資料量的問題**——放大四倍後判別力毫無變化，`N_TGE` 依然 `None`。
4. **E01 的 `N_TGE=475` 必須重新表述**：不是「`cnn_light` 能攻破加了遮罩的 AES」，而是「**在 profiling 與 attack 共用同一把金鑰的前提下**，`cnn_light` 能攻破加了遮罩的 AES」。這是只有換資料庫才會現形的可推廣性限制。

**尚未完成的部分（誠實列出）**：未來工作項目 2 原本包含三件事——variable-key（本次完成）、**非 ASCAD 資料集**（未開始）、**多個 byte（≥3–4 個）**（未開始）。後兩者都需要另外下載/準備資料（多 byte 需要 raw traces 重新抽取視窗，fixed-key 的 raw 檔約 5.6GB，variable-key 約 71GB），規模不小，本次未展開。

**另外，這裡沒有做的一件重要的事**：**沒有嘗試為 variable-key 尋找一組能work的配方**。本節只證明了「E01 的配方不能遷移」，沒有證明「variable-key 在本專案的框架下攻不破」——SCA 文獻普遍需要更大的模型與更多軌跡才能處理 variable-key，而本專案已經反覆證實（desync50/100、resnet、cnn_best）**換情境就必須重新調參，不能假設遷移**。若要讓 variable-key 成為防禦端可用的評估基礎，需要對它投入一輪與 B.7–B.16 同等規模的獨立調查，這是一筆明確、可預期、但尚未投入的成本。

### B.66 多 byte 分析（⚠ 核心結論已於 B.74 撤回）：byte 2 是一個**特例**，它的視窗剛好同時裝下兩個必要的洩漏——其他 byte 沒有這個運氣

> **⚠ 訂正（2026-09-16，見 B.74）**：本節的機制解釋**是錯的**。取得 Egger et al.（COSADE 2022）全文後發現，他們的附錄 A.1 表 4 公布了全部 16 個 byte 各自的單一 700 點視窗；實測他們的 byte 3 視窗 `[32906, 33606)`：遮罩值 SNR **8.56**、遮罩本身 SNR **1.25**，**兩個 share 都在同一個 700 點視窗內**，與 byte 2（6.59／1.33）幾乎相同。下面「任何 700 點視窗都裝不下」與「byte 2 是特例」的推論因此不成立——真正的原因是**我們自己把視窗開錯地方**（見 B.74）。本節其餘的實驗事實（E10 失敗、E10b 成功、E10c 對照）仍然成立，保留作為歷史記錄。

**動機**：資料集廣度的第二部分。E01 的配方是針對 ASCAD fixed-key **byte 2** 調出來的，而 ASCAD 官方發布的三個資料庫全部都是 byte 2 的 `[45400, 46100)` 視窗。要測其他 byte，必須自己從 raw traces 重新抽取——這是本節的主要工程。

**新增兩支腳本**：
- `scripts/06_find_byte_poi.py`：在 60000×100000 的 raw 檔上（分塊掃描，不整份載入）找出指定 byte 的 mask 欄位與洩漏位置。**兩件事都用實測決定、不靠推論**：mask 欄位掃過全部 16 欄並跟雜訊地板比對；洩漏位置用全解析度 SNR 曲線定位。
- `scripts/07_extract_window.py`：把指定的 raw 視窗抽成標準 ASCAD 格式（`Profiling_traces`/`Attack_traces` 各含 `traces`/`labels`/`metadata`，metadata dtype 與上游抽取檔完全一致，含補上的 `desync` 欄）。**不修改 vendored 的 `ASCAD_generate.py`**（它的標籤寫死 byte 2，且本專案的管線根本不讀 h5 裡的 `labels`——標籤一律由 `src/data/labels.py` 從 metadata 算，讓洩漏模型維持是 config 選項）。

**byte 3 的 POI 掃描結果**（8000 條子取樣）：

```
mask 欄位掃描 top4：[(1, 16.0973), (15, 0.6928), (0, 0.0528), (10, 0.0513)]  雜訊地板中位數 0.0483
=> 選中欄位 1，超出地板 333 倍；且與 B.61「fixed-key 把 byte 2..15 存在索引 0..13」的推論一致
   （byte 3 -> 欄位 1），這次是獨立驗證，不是沿用假設。欄位 15 再次出現次強訊號 = r_out，也與 B.61 一致。

masked-label SNR 峰值 16.0973 @ raw point 79702（對照組 0.0482，334 倍），11/100000 個點超過半高
```

**第一次嘗試（E10，單一 700 點視窗 `[79352, 80052)`，置中於 79702）**：`00_inspect_data.py` **通過**，而且數字比 byte 2 還漂亮——masked-label SNR 峰值 **15.4535**（byte 2 是 6.3003），對照組 0.0110，分離度 **1405 倍**（byte 2 是 600 倍），8/700 個點集中。正式訓練＋100-run 評估：

```
N_TGE = None   GE@9000 = 87.89   SR1 = 0.00   PI = -0.6555
（對照 E01/byte2：N_TGE=475、GE@9000=0.00、SR1=1.00）
```

**乍看的結論是「E01 的配方連換一個 byte 都撐不住」——這會是關於配方脆弱性最強烈的一個宣稱。動筆之前先做了一個確認，結果推翻了它。**

**根因：我的視窗漏掉了攻擊必需的第二個洩漏**。ID（遮罩未知）攻擊在數學上必須把**兩個**洩漏結合起來：遮罩後的值 `Z' = Z⊕r`，以及**遮罩 `r` 本身**（這就是二階攻擊）。實測兩個資料庫的 A 集：

| | 遮罩值 `Z⊕r` | **遮罩 `r` 本身** |
|---|---|---|
| byte 2（ANSSI 官方視窗） | 6.3003 @ 視窗點 517 | **1.2737 @ 視窗點 156**（在視窗內） |
| byte 3（我的單一視窗） | 15.4535 @ 視窗點 350 | **0.0106 @ 視窗點 276**（**雜訊層級，不在視窗內**） |

回到 raw 座標掃描 byte 3 的遮罩欄位（`masks[:,1]`）：**遮罩的洩漏峰值在 raw point 98863**（SNR 17.5595），而遮罩值在 **79702**——**兩者相距 19,161 個點**。~~**任何 700 點視窗都不可能同時裝下**。~~ **⚠ 這個推論錯了**：這兩個位置都在 masked SubBytes 區域**之外**（16 個 byte 的視窗最遠只到 raw 50265），是該中間值在後續運算中的殘留；在 SubBytes 區域內、byte 3 自己的處理視窗裡，兩個 share 都在（B.74）。

**所以 E10 的失敗不是配方的問題，是我抽取視窗的錯誤**——那個視窗讓攻擊在資訊論上就不可能成立，跟超參數毫無關係。**這個錯誤的結論沒有被寫進任何地方，因為在下筆前先做了這項確認。**

~~**真正的發現，比原本以為的更有意思**：**byte 2 是一個特例**。~~（**⚠ 已撤回，見 B.74**；以下段落保留原文作為記錄）ANSSI 選的 `[45400, 46100)` 之所以能用單一 700 點視窗支撐 ID 攻擊，是因為 byte 2 的遮罩（raw 45556）與遮罩值（raw 45917）**剛好只差 361 個點**。byte 3 差了 19,161 點。換句話說——**整個 ASCAD 生態系（以及本專案至今的一切）建立在一個運氣很好的 byte 上**；「換一個 byte 重跑」這件事，並不是換個 `target_byte` 參數那麼簡單，而是需要重新做 POI 工程。

**修正做法**：`07_extract_window.py` 改成支援 `--windows START:STOP [START:STOP ...]`，把多個視窗串接起來。byte 3 用兩個 700 點視窗（各自置中於兩個 POI）串成 1400 點：

```
byte3 雙視窗（1400 點）驗證：
  遮罩值 Z^r     : 峰值 15.4535 @ 點 350
  遮罩 r 本身    : 峰值 17.4826 @ 點 1050
  未遮罩 Z（對照）: 峰值  0.0110 @ 點 245
  => 兩個洩漏都在視窗內，且都比 byte 2 對應的數字更強（6.3003 / 1.2737）
```

**必須誠實標註的差異**：`trace_len` 因此是 1400 而非 700，`cnn_light` 的參數量隨之從 18,640 變成 32,640（flatten→FC 層與輸入長度成正比）。這是「需要兩個視窗」這個資料事實逼出來的，不是調參選擇，但**「同一套配方」在這裡不是字面上同一個模型**，比較時必須說清楚。

### B.67 修正後的 byte 3 結果：配方完全遷移得過去，而且 byte 2 其實是個**偏難**的目標

**E10b（byte 3，雙視窗 1400 點，配方與 E01 完全相同）正式 100-run 評估**：

```
N_TGE  = 8        N_SR90 = 11       PI = +1.0077 bits
GE @ N=10   = 0.27  SR1 = 0.87
GE @ N=50   = 0.00  SR1 = 1.00      （之後全程維持）
```

**8 條軌跡就穩定破解金鑰**——對照 E01（byte 2）的 `N_TGE=475`，快了 **59 倍**；`PI` 也從 E01 的 **−0.4999 轉為 +1.0077**（本專案除了「評估者上界」ID_MASKED 之外，唯一 PI 明顯為正的真實攻擊結果）。

**先排除混淆變因再下結論**。`N_TGE=8` vs `475` 同時混了三件事：byte 不同、視窗長度不同（1400 vs 700）、模型大小不同（32,640 vs 18,640 參數）。因此做了 **E10c 對照組**——把 byte 2 也抽成 1400 點（`[45036, 46436)`，同時涵蓋它的兩個 POI；實測 SNR 與原視窗完全相同：`Z^r`=6.3003、`r`=1.2737），其餘一切（配方、seed、模型大小、評估協定）與 E10b 完全一致，**唯一變因是 byte**：

| 實驗 | byte | 視窗 | trace_len | 參數量 | N_TGE | GE@9000 | SR1 | PI |
|---|---|---|---|---|---|---|---|---|
| E01 | 2 | ANSSI 700 點 | 700 | 18,640 | 475 | 0.00 | 1.00 | −0.4999 |
| E10（**瑕疵**） | 3 | 單一 700 點 | 700 | 18,640 | None | 87.89 | 0.00 | −0.6555 |
| **E10b** | **3** | **雙視窗 1400 點** | 1400 | 32,640 | **8** | **0.00** | **1.00** | **+1.0077** |
| **E10c（對照）** | **2** | 加寬 1400 點 | 1400 | 32,640 | **None** | 100.86 | 0.00 | −0.2147 |

**對照組給出乾淨的答案**：同樣 1400 點、同樣 32,640 參數、同一套配方，**byte 3 用 8 條軌跡破解，byte 2 則完全沒有收斂**。所以 byte 3 的優勢**不是**來自比較長的視窗或比較大的模型——**加寬視窗反而讓 byte 2 變差了**（700 點時 `N_TGE=475`，1400 點時變成 `None`）。

**四個結論**：

1. **E01 的配方在 byte 之間遷移得非常好**。這是本專案第一次「換情境還能用」的正面案例——先前換 desync 幅度（B.18/B.25–B.28）、換架構（B.50、B.56–B.60）、換資料庫（B.64）全部失敗，而換 byte 不但成功，還是全專案最強的真實攻擊結果（`N_TGE=8`）。**配方的脆弱性因此可以被更精確地界定：它對「訊號的時間結構與統計特性」敏感（desync、變動金鑰），但對「同一份採集裡換一個目標中間值」不敏感。**
2. **byte 3 的優勢是它本身洩漏就強得多**，尤其關鍵的是**遮罩的洩漏強了 13.7 倍**（17.4826 vs 1.2737；⚠ 這是拿我們錯誤視窗裡的 r₃ 殘留跟 byte 2 視窗裡的 r₂ 相比，不是同一類位置的公平比較，見 B.74）。ID 攻擊本質上是二階攻擊，必須同時抓到遮罩值與遮罩；byte 2 的遮罩洩漏（1.2737）相對微弱，正是它難攻的原因。
3. ~~**byte 2 其實是一個「雙重特殊」的目標**~~（**⚠ 已撤回，見 B.74**）：既幸運（兩個 POI 只差 361 點，單一 700 點視窗就裝得下——其他 byte 沒這運氣，byte 3 差了 19,161 點），又**偏難**（洩漏強度在測過的兩個 byte 裡明顯較弱）。**整個 ASCAD 生態系與本專案至今的全部結論，都建立在這一個既方便又偏難的 byte 上。**
4. **視窗不是越寬越好**：把 byte 2 從 700 點加寬到 1400 點，在資料量不變的情況下，模型變大、無關的點變多，結果從 `N_TGE=475` 退化成完全不收斂。這與 B.8/B.42/B.49 反覆看到的「排程長度／epoch 數／容量都必須跟其他條件一起調」是同一類教訓。

**方法論價值（值得寫進報告）**：本節是本次投入中**最接近發表一個錯誤結論的一次**。E10 的失敗（`N_TGE=None`）本來會被寫成「E01 的配方連換一個 byte 都撐不住」——這會是關於配方脆弱性最強烈、也最吸引人的宣稱，而且它有完整的實驗數據支持、`00_inspect_data.py` 甚至還 **PASS** 了（SNR 15.4535、分離度 1405 倍，比 byte 2 還漂亮）。**健檢通過並不代表視窗可用**：`00_inspect_data.py` 只檢查「遮罩已知標籤的 SNR 是否顯著高於未遮罩對照組」，而這個檢查對「遮罩本身有沒有落在視窗內」完全盲目——但後者才是 ID 攻擊能否成立的前提。真正救回來的，是動筆前多問一句「這個視窗裡到底有沒有攻擊所需的全部資訊」。

**由此產生的一個具體改進建議（尚未實作）**：`00_inspect_data.py` 應該增加一項檢查——**同時量測遮罩本身的 SNR**，並在「遮罩值有洩漏但遮罩沒有」時發出警告，因為那正是「視窗看似健康、ID 攻擊卻不可能成立」的特徵。這個檢查若早已存在，E10 那次無效的訓練與評估就會在資料階段就被攔下來。


## 真實硬體應用：把現有防禦成效搬到 ChipWhisperer 實測軌跡上

### B.68 E11：第一個非 ASCAD、實體量測的資料庫，以及 PSR 作為成本指標在硬體上的失效

**動機**：本專案（含防禦端）至今每一個結論都建立在 ASCAD 這個「下載得到的」資料庫上。舊專題資料夾裡其實存有 **10,000 條真實 ChipWhisperer 採集軌跡**（`B11209017/ASCAD/zaid/data/cw_traces.npy`，1500 點，float64，CW ADC 範圍 ±0.5；固定金鑰 `2b7e151628aed2a6abf7158809cf4f3c`，隨機明文；16 個 byte 的 Zaid 架構模型也都訓練過）。把現有防禦搬到這批實測軌跡上，等於把「在公開資料庫上驗證」升級成「在我們自己的物理量測上驗證」，這是應用面最值得主張的一步。

**匯入時要處理兩個與 ASCAD 的差異**（`scripts/08_import_cw.py`）：

1. **dtype/尺度**：`src/data/ascad.py::load` 寫死 `dtype=np.int8`，而 CW 存的是 float64、範圍 ±0.5——**直接餵進去會被截斷成全 0，資料完全銷毀**。因此匯入時做顯式線性縮放到完整 int8 範圍。量化代價可忽略：步階為單條軌跡標準差的 **2.45%**，而 ID-label SNR 有 6–12.6。
2. **沒有遮罩**：這是未加保護的實作，`Sbox[p⊕k]` 直接一階洩漏。寫入全零的 masks 欄位只為維持檔案格式一致，並讓 `00_inspect_data.py` 偵測到「所有 masks 欄位皆為常數」時**切換到未保護裝置的檢查邏輯**（改查 ID 標籤本身是否洩漏——在遮罩裝置上這個量是趨近 0 的對照組，在這裡卻必須是強訊號）。

**健檢通過**：ID-label SNR 峰值 **11.9890 @ point 1342**，雜訊地板 0.0481（**249 倍**），12/1500 個點超過半高，金鑰 `0x2b` 由 metadata 讀出。

**E11 基準攻擊者**（E01 配方原封不動，A=6000/V=1000/D=1000/E=2000）：

```
N_TGE = 1       N_SR90 = 2      PI = 6.1977 bits（滿分 8）
GE @ N=2000 = 0.0000            SR1 = 1.0000
```

**一條軌跡就破解金鑰**——未加遮罩的軟體 AES 的教科書式一階攻破。這也讓它成為**理想的防禦展示基準**：未防禦時攻擊強到極限，任何防禦效果都能乾淨量測出來。（注意：`N_TGE=1` 遠低於 ASCAD 的任何結果，這反映的是**裝置更容易攻**，不是配方更好，兩者不可比。）

**把兩個物理上可部署的防禦套上去**（`05_apply_defense.py`，同一套管線、同一個凍結攻擊者、100-run 正式評估）：

| 防禦 | PSR | N_TGE | 相對未防禦 | GE@2000 | PI |
|---|---|---|---|---|---|
| 無（基準） | 0 | **1** | — | 0.00 | +6.1977 |
| gaussian σ=0.25 | 0.0094 | 2 | 2× | 0.00 | +5.8534 |
| gaussian σ=0.5 | 0.0189 | 2 | 2× | 0.00 | +4.4769 |
| gaussian σ=1 | 0.0378 | 3 | 3× | 0.00 | −2.2049 |
| gaussian σ=2 | 0.0755 | 11 | 11× | 0.00 | −19.2326 |
| gaussian σ=4 | 0.1511 | 64 | 64× | 0.00 | −34.8171 |
| jamming shift5 | 0.6354 | 222 | 222× | 0.00 | −108.2765 |
| jamming shift20 | 0.8011 | 799 | 799× | 0.00 | −118.5661 |
| **jamming shift50** | 0.9241 | **1314** | **1314×** | 0.00 | −121.5392 |

**乾淨的單調劑量反應曲線，沒有異常點**——防禦成效在真實量測軌跡上完全成立，不是模擬資料庫的產物。（所有配方在 2000 條窗口內最終仍被攻破，這是因為裝置未加保護、且攻擊集只有 2000 條；`N_TGE=1314` 已逼近可量測上限。）

### B.69 這批實測揭露的一個方法論問題：PSR 在硬體情境下會嚴重誤導

本專案一路用 **PSR**（擾動的 L2 相對於乾淨訊號的 L2）當防禦成本的主指標。在 ASCAD 的離線後製情境下這沒問題。**搬到真實裝置上就出問題了**：

| 防禦 | PSR（我們的成本指標） | **裝置實際付出的代價** | 換到的攻擊成本 |
|---|---|---|---|
| gaussian σ=4 | 0.1511（看起來便宜） | 必須真的產生並注入**振幅達訊號 15% 的雜訊功率** | 64× |
| **jamming shift50** | **0.9241（看起來幾乎災難級）** | **執行時間慢約 3.3%**（1500 取樣點中的 50 點），**功耗幾乎不變** | **1314×** |

**PSR 把 jamming 排成比高斯貴 6 倍，但在真實部署裡 jamming 便宜得多、效果還好 20 倍。** 原因是 PSR 量的是「防禦後軌跡與乾淨軌跡的向量距離」——一條被平移 5 個取樣點的軌跡，與原軌跡的 L2 距離很大，但**裝置消耗的功率幾乎完全相同**，它只是沒對齊而已。

**結論：成本指標必須反映裝置付出的東西（功耗、時間、面積），而不是兩條軌跡向量之間的距離。** 對時域類防禦（random delay / jamming / shuffling），正確的成本是**執行時間 overhead**；對振幅類防禦（噪訊注入）才是功率。用單一個 PSR 同時衡量兩者，會系統性高估時域防禦的成本。

**這一點只有在思考真實部署時才會浮現**，是把工作搬上硬體最直接的方法論收穫，也直接呼應方向.md 說的「整個對抗式 SCA 防禦文獻的共同弱點就是全部停在離線後製」——**停在離線後製時，你不會發現自己的成本指標是錯的**。

### B.70 應用面範疇的誠實界定：哪些防禦真的能部署，哪些不能

把防禦搬上硬體時，必須區分兩類：

| 類別 | 防禦 | 可否部署 | 理由 |
|---|---|---|---|
| 不需觀測軌跡 | 高斯噪訊、jamming（隨機延遲） | **可以** | 擾動與當下軌跡內容無關，韌體/電路端直接施加即可（隨機延遲更是 SCA 領域的經典對策） |
| 需觀測軌跡 | D01–D05（learned generator） | **不行** | 這些是 `擾動 = G(軌跡)`，而軌跡正是要保護的東西，它在發生的當下才存在——**無法先觀測再回頭擾動它** |

**這解釋了 D01–D05 這一整類方法的本質限制：它們是離線評估用的建構，不是可部署的對策。** 也正好說明為什麼方向.md 的 Route C 建議「把 G 蒸餾成 universal perturbation 模板」再注入——那是為了迴避這個部署性問題。**但附錄D 已經代數證明：對每條軌跡加同一個常數，SNR 與 TVLA 精確不變，防禦效益為零。** 換句話說，**「可部署的蒸餾版本」與「有效的原版本」在目前的設計下無法兼得**，這是一個真正的開放問題，不是實作細節。

**因此本次硬體應用只測了兩個「不需觀測軌跡」的防禦**——這不是偷懶，是範疇上的必然。而結果顯示：**光是這兩個可部署的防禦，就能把未防禦時 1 條軌跡即破的攻擊，推到 1314 條**（jamming shift50，實際代價約 3.3% 執行時間）。這本身就是一個完整、誠實、可展示的應用面成果。

**尚未做（明確列出）**：韌體端真正實作隨機延遲並**重新採集**（目前的 jamming 是對已採集軌跡做後製位移，效果等價但不是真的部署）；以及功耗/面積的實測 overhead。前者是 CW 官方教材範圍內的工作，是這條路線最自然的下一步。

### B.71 把每一個結果都畫成圖：兩層圖表，以及畫圖過程中抓到的三個「圖會說謊」的坑

**做法分兩層**：

1. **每次執行一張**（`04_make_report.py`）：掃過 `runs/`、`runs_byte3/`、`runs_cw/`、`runs_variable/`、`runs_resnet_{robust,patience,ctrl1ep}/`、`defenses/`、`defenses_cw/`，替 **126 個有 `metrics.json` 的 run** 各補上 `figures/ge_curve.png`（含 25/75 百分位帶）與 `figures/sr_curve.png`，並把全部 126 列彙成單一張 `reports/comparison.md` / `.tex`。順手把 `--runs-dir` 改成可以吃多個目錄——原本一個目錄一張表，寫出去會落在 `reports/per_dir/runs*/`，而 `.gitignore` 的 `runs/`、`runs_*/` 規則**沒有錨定在根目錄，會在任何層級生效**，等於整批表格被靜默吃掉。這是這次才發現的一個小陷阱。
2. **跨實驗、帶論點的結果圖**（新增 `scripts/09_make_figures.py`）：14 張 F01–F14 寫進 `reports/figures/`（進版控，清單見該目錄的 `README.md`），對應 B.7 到 C.3 的每一條調查主線。

**設計上的一條硬規則**：圖上每條曲線、每個數字都從 `metrics.json` / `train_history.csv` / `cost_metrics.json` 讀回來，**不從本文件手抄**，這樣圖不可能跟產生它的那次執行脫節。只有兩個例外，且都在圖上與腳本 docstring 標明：F05 與 F12 左圖從 `.h5` 重算 SNR（SNR 是資料的性質，不屬於任何一次 run）；F11 右圖的四個 SNR 峰值是 B.66/B.67 的轉錄值，因為 byte 3 的資料庫在 GPU server 上、不在本機 `data/`。

**畫圖過程抓到三個「圖表本身會誤導」的問題，都不是程式 bug，是視覺化選擇的問題**——值得記錄，因為它們跟本專案一路在防的「量測方法讓結論失真」是同一類錯誤，只是換到了呈現層：

| # | 原本的畫法 | 為什麼會誤導 | 改成 |
|---|---|---|---|
| 1 | F02 用 **GE@9000** 比較 one-cycle 三個維度 | 所有收斂的設定 GE@9000 **全部是 0.00**，圖上完全分不出 475 跟 4253 的差別，等於把 B.13–B.15 的整個結論畫成一片平地 | 改用 **N_TGE**（未收斂者畫到窗口天花板並標示 never converged），這才是附錄實際在比的量 |
| 2 | F08 右圖 loss 軸自動縮放到 5.47–5.55 | 在這個放大倍率下，cnn_best 的 0.07 降幅看起來像是明顯的學習曲線，**直接反駁自己的標題**「loss never leaves the random baseline」 | 軸固定 5.0–5.62，並疊上 **cnn_light（E01）真的有在學** 的 loss 曲線當比例尺：同一張圖上 0.07 vs 0.49，一眼看得出差別 |
| 3 | F09 中間圖把 patience=6 與 patience=100 兩組**各自排序後**配對 | 這樣畫出來必定是一條完美對角線，**即使兩組其實毫無關聯**——圖會「證明」一個它根本沒檢定的東西 | 從各 run 的 `config_snapshot.yaml` 讀回 `seed`，**依種子配對**（新增 `_seed_of()`），配對平均差直接標在標題上 |

第 3 點特別值得警惕：那張圖原本看起來完全正確、也確實支持 B.58 的正確結論，但它的**證據強度是假的**。這跟 B.24/B.28/B.35/B.50 反覆遇到的「20-run 預覽被雜訊騙到」是同一個模式的另一種形態——**一個看起來支持結論的東西，不代表它真的檢定了那個結論**。

**另外兩處把數字改成從資料現算，而不是寫死**：F12 原本標題寫「leaks 1000x harder」（順手取的整數），實際重算是 175 倍，已改成由 `peaks` 字典現算後填入；F13 原本寫 jamming「~28x cheaper」，那是拿 PSR 0.151 跟執行時間 0.033 相除——**兩個不同單位的量相除沒有意義**，已改成只陳述各自的實測值（PSR 排序差 6 倍、裝置端代價 3.3% 執行時間、效果差 20 倍）。

`tests/` 60 個測試在改動後全數重跑通過（新增的兩支腳本不觸碰 `src/`，只讀既有產物）。

### B.72 自適應攻擊者（A1）評估一直有訓練／推論前處理不一致的 bug——修正後 D01／D02_noreg／D05 被攻破得更快，D04 的結論反而更強

**怎麼發現的**：為了讓 Zaid 強攻擊者也跑自適應測試（B.73），重讀 `02_run_attack.py` 時發現：它在推論前會**從乾淨的 `.h5`** 取 A 集重新 fit Standardizer／MinMaxScaler。但自適應攻擊者是用 `01_train_attacker.py --profiling-traces` 在**防禦後**的 profiling 軌跡上訓練的——訓練時的標準化統計量來自防禦後的 A，推論時卻換成乾淨 A 的統計量。模型吃到的輸入尺度跟訓練時不一樣。

**先確認這不是紙上談兵**（本機，同一個模型、同一批軌跡）：把 E 集與 profiling 集同時加上常數 25，
- 舊行為（scaler 從乾淨 A fit）：輸出機率與基準不一致，最大差 0.104；
- 修正後（scaler 從同樣加了 25 的 A fit）：與基準逐位一致；
- 把乾淨 A 透過新參數傳入：與原本行為逐位一致（修正不改變非 A1 的任何結果）。

**修正**：`01_train_attacker.py` 在 `--profiling-traces` 時把絕對路徑寫入 `config_snapshot.yaml` 的 `data.profiling_traces_path`；`02_run_attack.py` 新增 `--profiling-traces`，並在未指定時自動讀取該欄位，從同一批軌跡取 A 重新 fit。舊的 run 沒有這個欄位，需明確傳入。

**對既有 15 個 cnn_light A1 結果重新評估**（不重新訓練，只重跑推論與 100-run 評估；原本的 `metrics_vs_*_defended.json` 保留，修正版寫在旁邊 `*_refit.json`）：

| 配方 | seed | GE@9000 舊 → 修正 | N_TGE 舊 → 修正 | SR1 舊 → 修正 |
|---|---|---|---|---|
| D01 | 42 | 0.00 → 0.00 | **596 → 263** | 1.00 → 1.00 |
| D01 | 123 | 122.18 → **63.54** | None → None | 0.00 → 0.00 |
| D01 | 777 | 0.00 → 0.00 | **5306 → 2664** | 1.00 → 1.00 |
| D02_noreg | 42 | 0.04 → 0.00 | **6268 → 339** | 0.96 → 1.00 |
| D02_noreg | 123 | 103.59 → 118.97 | None → None | 0.00 → 0.00 |
| D02_noreg | 777 | 33.64 → **3.45** | None → None | 0.00 → 0.09 |
| D03_high | 42 | 17.99 → 15.63 | None → None | 0.00 → 0.00 |
| D03_high | 123 | 43.09 → 47.88 | None → None | 0.00 → 0.00 |
| D03_high | 777 | 132.62 → 127.97 | None → None | 0.00 → 0.00 |
| **D04** | 42 | 124.17 → **174.47** | None → None | 0.00 → 0.00 |
| **D04** | 123 | 137.68 → **151.07** | None → None | 0.00 → 0.00 |
| **D04** | 777 | 49.31 → **61.37** | None → None | 0.00 → 0.00 |
| D05 | 42 | 0.00 → 0.00 | **597 → 276** | 1.00 → 1.00 |
| D05 | 123 | 219.60 → 210.26 | None → None | 0.00 → 0.00 |
| D05 | 777 | 7.17 → 15.18 | None → None | 0.00 → 0.00 |

**三個結論**：

1. **這個 bug 系統性地讓自適應攻擊者看起來比實際弱。** 對擾動較大的配方影響最明顯：D02_noreg seed 42 從 6268 條降到 339 條（快 18 倍），D01、D05 都快了約一倍。PI 也大幅改變（D01 seed 42：−0.889 → −0.076），先前在防禦端報告裡解讀的「PI 深度為負 = confidently wrong」有一部分其實是前處理尺度錯位的產物。
2. **防禦端報告的案例 3（「D02_noreg 的 TVLA 與 D01 相近，自適應抵抗力卻強 10.5 倍」）不成立。** 修正後 D02_noreg 339 條、D01 263 條，兩者同一量級；D02_noreg seed 777 甚至 GE 降到 3.45、SR1 出現 9%。那個「TVLA 與自適應抵抗力不同步」的反例，是這個 bug 製造的。
3. **D04 的結論沒有被削弱，反而更強。** 三個種子 GE 全部上升（49.31 → 61.37 是最差種子），N_TGE 全部仍為 None。D03_high 三個種子也仍未收斂，但最差種子 15.63 且曲線持續下滑。

**同一個 bug 也影響 D04＋jamming 的 A1（防禦端附錄 U）與 resnet 的 A1**，已以同樣方式重新評估，結果見 B.73 之後的記錄。

**方法論價值**：這是本專案第三次「為了做 A，發現 B 早就錯了」（前兩次見 B.56、B.63）。這次的錯誤特別隱蔽：訓練、推論、評估三支腳本各自都是正確的，錯在它們之間的**隱含約定**——`02_run_attack.py` 的註解寫著「A 上的前處理是確定性的，重算即可重現訓練時的前處理」，這在 `--profiling-traces` 加入之前完全成立，加入之後就悄悄失效了。新增一個參數時，下游所有「重新推導而非讀取」的步驟都要一起檢查。

### B.73 重現文獻強攻擊者：Zaid 模型 5 個種子中 3 個達到甚至超過論文數字，另 2 個完全沒學起來；Wouters noConv1 在本管線下無法重現

**動機**（`dlsca-defense-v2/docs/文獻查核.md` §3.1 第 5 點）：E01 的 N_TGE=475，而 Zaid et al.（TCHES 2020(1)）在同一資料集報告 N̄_tGE=191。防禦只對一個比已發表結果弱的攻擊者驗證過，不足以作為證據。

**先釐清一件本文件過去寫錯的事**：`cnn_light` 並不是 Zaid 的模型。B.8 說它「師承 Zaid」，但實際來源是團隊自有的 `train_cnnd.py`（B.11），它多一個 Conv(8, k=51) 區塊、第一層卷積核為 3。Zaid 的 ASCAD N0=0 模型是 Conv1D(4, k=1)－BN－AvgPool(2)－FC10－FC10－FC256，16,960 參數（`src/models/zaid_ascad.py` 從作者 repo 的 `ASCAD/N0=0/cnn_architecture.py` 逐層移植，參數量精確吻合）。

**實作**：
- `src/models/zaid_ascad.py`：`zaid_ascad0`（16,960 參數）、`wouters_noconv1_ascad0`（6,436 參數，Wouters et al. Table 1 吻合）。
- `train.selection.metric: final_epoch`：Zaid 的原始腳本沒有 checkpoint callback，訓練 50 epoch 取最後權重。GE 預覽仍記錄，但不選模、不早停，避免「重現」偷偷加上原作沒有的選模步驟（`tests/test_callbacks.py` 新增護欄）。
- `keyrank.n_tge_per_run` 與 `metrics.json` 的 `n_tge_per_run`：Zaid 報告的是**每次攻擊各自的 N_tGE 取平均**，不是本專案一直用的「平均 GE 曲線其後持續 <1」。兩者並列，才能拿同一種定義比較。
- `split.n_defender` 允許為 0（Zaid 用 45k 訓練／5k 驗證，沒有防禦者集合）。
- `preprocess.method: horizontal_standardize`：Wouters 的原始前處理（逐條軌跡 z-score，見其 `preproces.py`）。

**Zaid 結果**（ASCAD fixed-key desync0，E 集 10,000 條，9,000 條窗口，100 次攻擊）：

| 切分 | seed | N_TGE（曲線） | 每次攻擊 N_tGE 平均（Zaid 定義） | GE@9000 | 最終 train loss |
|---|---|---|---|---|---|
| 45k/5k（原作） | 42 | None | —（0/100 收斂） | 216.23 | 5.5311 |
| 45k/5k | 123 | None | — | 238.64 | 5.5421 |
| 45k/5k | **777** | **93** | **88.7** | 0.00 | 5.2451 |
| 45k/5k | **1** | **123** | **102.6** | 0.00 | 5.2293 |
| 45k/5k | **2024** | **107** | **88.3** | 0.00 | 5.2626 |
| 30k/5k/15k（本專案切分） | 42 | None | — | 128.25 | 5.5138 |
| 30k/5k/15k | 123 | None | — | 182.53 | 5.5068 |
| 30k/5k/15k | **777** | **235** | **192.2** | 0.00 | 5.2006 |
| 30k/5k/15k | **1** | **261** | **219.5** | 0.00 | 5.4084 |
| 30k/5k/15k | **2024** | **132** | **133.5** | 0.00 | 5.2092 |

**三個結論**：

1. **重現成功，但只在 3/5 個種子上**。成功的種子在原作切分下 N̄_tGE 為 88.3–102.6，**比論文的 191 還好**；在本專案較小的 30k 切分下為 133.5–219.5，與論文同一量級。無論哪一種切分，成功的種子都比 E01（475）強 2–5 倍。
2. **失敗的種子不是「學得比較差」，是完全沒學**：最終 train loss 停在 5.51–5.54（隨機基準 log 256 = 5.545），GE 甚至高於隨機基準（216–239）。兩種切分中失敗的都恰好是 seed 42 與 123——論文只報告一個數字，沒有報告種子成功率。這與 B.57–B.59（resnet 失敗率 50%）是同一類現象，**第零層檢查在這裡必須逐種子執行**：只有在乾淨資料上真的恢復出金鑰的種子，才能拿來衡量防禦。
3. **E01 之前被當作「專案的基準攻擊者」，但它比這個 17K 參數的公開模型弱得多**，防禦端所有以 E01 為凍結攻擊者的結論，都必須以成功的 Zaid 種子重新檢驗（進行中，見防禦端附錄 Y.5）。

**Wouters noConv1：本管線下無法重現（10 個種子、兩種前處理，全部失敗）**。第一次沿用 Zaid 的逐點 Standardizer＋MinMax，全部 GE>200；查原作 repo 發現 Wouters 實際用的是逐條軌跡標準化（其論文的論點正是「給定適當前處理，第一層卷積可以移除」），改用後重跑，**仍全部失敗**，最終 train loss 5.541–5.543，完全沒離開隨機基準。尚未排除的差異：Keras 版本（原作為 Keras 2）、Dense 層預設初始化、其 one-cycle 實作與本專案 `OneCycleLR` 的細節。**不宣稱 Wouters 的結果不成立**——只能說在本專案的管線與 10 個種子下沒有重現，因此不把它用作防禦評估的攻擊者。

**附帶的一個乾淨驗證**：Zaid 攻擊者在 D09 additive（Gu et al. 式通用擾動）防禦後的軌跡上重訓練，與同種子在乾淨軌跡上訓練的結果**實質相同**（GE 曲線最大差 0.04、N_TGE 與 PI 相同到小數點後四位）。原因是逐點標準化會減去每個時間點的平均值，常數擾動在進入模型前就被消掉了——對任何做逐點標準化的自適應攻擊者，通用加性擾動在定義上就是無效的。

**B.73 補充：以重現的強攻擊者檢驗防禦端結論**（完整數字記錄於 `dlsca-defense-v2/CLAUDE.md` 附錄 Y.5–Y.6）

- **第一層（凍結攻擊者，84 組評估）**：只用在乾淨資料上真正學起來的 Zaid seed 777／1／2024。D01、D05 在 E01 下「守住」，換成 Zaid 後只需 154–196 與 97–131 條，與無防禦（93–123）幾乎相同——與它們 TVLA 幾乎不變一致。D04 三種子全被攻破，但需 2,242–4,448 條（同種子無防禦的 18–44 倍，同成本高斯只有 1.1–2.1 倍）。
- **第三層（Zaid 自適應，D04）**：seed 777 以 **140 條**完全攻破；seed 2024／1 的 GE 為 44.72／39.85；seed 42／123 重訓練後 loss 仍在 5.542，屬無效攻擊者、不計入。
- **自適應評估也要逐種子做第零層檢查**：把學不起來的種子算進去，D04 會被誤判為「5 個中 4 個未攻破」。同理，Gu 式插入防禦（L=4）的「未攻破」，大部分來自自適應攻擊者訓練失敗（6 個中 3 個 loss 未離開 log 256）。
- 新增 `configs/exp/E12_adaptive_A1_zaid.yaml`；自適應攻擊者結果存於 `runs_adaptive_gu/`（gitignored）。

### B.74 訂正 B.66／B.67：byte 3 的兩個 share 其實在同一個視窗裡，錯的是我們挑視窗的方法

**起因**：使用者取得 Egger, Schamberger, Tebelmann, Lippert, Sigl. *A Second Look at the ASCAD Databases*（COSADE 2022, LNCS 13211, pp. 75–99）全文。這篇正是 B.66 當初無法核實、防禦端 `docs/文獻查核.md` 列為「優先取得」的那一篇。

**它說了什麼**：

- **附錄 A.1 表 4 公布了全部 16 個 byte 各自的單一視窗**（ASCAD_fix 700 點、ASCAD_variable 1400 點）。byte 3 的 fix 視窗是 `[32906, 33606)`，byte 2 是 `[45400, 46100)`（即 ANSSI 的官方視窗）。他們的對齊基準是 S(p⊕k)⊕r_out 的峰值，固定在視窗點 492。
- **表 1 是在那些逐 byte 視窗上算的**（表 2 才是全長）。在 byte 3 的 700 點視窗內，二階多變量 CPA **280 條軌跡**即恢復金鑰，二階單變量 2,060 條。也就是說，單一 700 點視窗對 byte 3 **足以支撐二階攻擊**。
- 他們也指出 ASCAD 視窗的設計意圖本來就是同時包含遮罩值與遮罩（rationale 明確寫出 r_out 與 r_i），以及各 byte 視窗內的洩漏組成不同（k2、k3 額外有 r_in，k4 有 r_out，k5 有 r_i）。
- 結論直接寫著：單一 byte 的結果可能不具代表性，攻擊者必須為每個 byte 各訓練一個模型。

**我們的實測（8,000 條 raw 軌跡，`check_egger_windows.py`）**：

| 視窗 | 遮罩值 Z⊕r₃／Z⊕r₂ | 遮罩本身 r₃／r₂ | 判定 |
|---|---|---|---|
| byte 2 ANSSI `[45400,46100)` | 6.5922 @ raw 45917 | 1.3272 @ raw 45556 | 兩個 share 都在 |
| **byte 3 Egger `[32906,33606)`** | **8.5579 @ raw 33420** | **1.2522 @ raw 33088** | **兩個 share 都在** |
| byte 3 我們的 `[79352,80052)` | 16.2306 @ raw 79702 | **0.0392（雜訊）** | 只有一個 share |

**所以 B.66 的核心主張是錯的**：byte 3 的兩個 share 並非「相距 19,161 點、任何 700 點視窗都裝不下」，而是好端端地一起待在它自己的 SubBytes 處理視窗裡，結構與 byte 2 幾乎相同。

**錯在哪裡**：`06_find_byte_poi.py` 掃過整條 100,000 點軌跡，取 Z⊕mask 的 SNR 全域最大值當作 POI。byte 3 的全域最大值（16.23 @ raw 79702）**不在 SubBytes 區域內**——用 Egger 表 4 對照，16 個 byte 的視窗最遠只到 **raw 50265**，79702 與 98863 都在那之後，是該中間值在後續運算裡的殘留（Egger 註明暫存器不清除，S(p⊕k)⊕r 會延續到後面）。我們把視窗開在那個「訊號最強但只有一個 share」的地方，於是做出一個資訊論上不可能成功的攻擊，再把失敗歸因給 byte。

**用 Egger 視窗重跑（E10d，`configs/exp/E10d_byte3_egger.yaml`，三個種子，其餘與 E01 完全相同）**：

```
00_inspect_data.py：masked-label SNR 8.5515 @ 514，mask-value SNR 1.1722 @ 182，PASS
seed 42 ：GE@9000=1.85   N_TGE=None  SR1=0.25  每次攻擊收斂 25/100（平均 7,856 條）  PI=+1.0216
seed 123：GE@9000=11.71  N_TGE=None  SR1=0.00  0/100                                  PI=+0.9183
seed 777：GE@9000=8.77   N_TGE=None  SR1=0.00  0/100                                  PI=+0.9674
```

**修正後的三段式結論**（兩邊都不要過度修正）：

1. **「沒有任何 700 點視窗裝得下 byte 3 的兩個 share」——撤回。** Egger 的視窗裝得下，我們自己的健檢也確認（mask-value SNR 1.17）。
2. **「byte 2 是幸運的特例」——撤回。** byte 2 的視窗結構跟 byte 3 的並無本質差異，兩者都是 ANSSI／Egger 依同一個對齊規則選出來的。
3. **「雙視窗遠優於單視窗」——仍然成立，但原因不同。** E10b 的雙視窗 N_TGE=8，而 Egger 單視窗三個種子都沒在 9,000 條內收斂（GE 1.85–11.71）。原因不是「單視窗缺一個 share」，而是我們的雙視窗把**訊號更強的殘留位置**（16.23）跟一個強遮罩洩漏拼在一起，SNR 條件優於 Egger 視窗的 8.56／1.25。E10 原本的失敗（GE 87.89）則確實是缺 share 造成的。

**真正該留下的教訓，是關於工具鏈而不是關於 ASCAD**：二階攻擊的視窗挑選，必須**對兩個 share 各驗一次**。B.67 之後我們確實把這個檢查加進了 `00_inspect_data.py`（mask-value SNR），但只用在「視窗抽完之後」，沒有套用到「選視窗」那一步——而選視窗正是用單一標籤的 argmax 做的。一個只檢查下游、不檢查上游的護欄，擋不住上游的錯。

**文獻定位的連帶影響**：多 byte 這條線不再有可主張的新發現（Egger 已做且做得更完整：全 16 byte、逐 byte 視窗、CPOI 與組合語言對應、跨 byte 訓練遷移矩陣）。防禦端 `docs/文獻查核.md` 第 8 項與第 4 節「不要主張的」已同步更新。
